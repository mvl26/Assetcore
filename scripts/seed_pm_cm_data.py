...
ASSET_IDS = []  # will be populated from DB
...
def seed_pm_work_orders(assets: list[str], technician: str):
    ...
    for i, (a_idx, status, due_offset, is_late, pm_type, result, notes) in enumerate(specs):
        ...
        due_date = add_days(today, due_offset)
        doc = frappe.get_doc({
            ...
            "is_late": is_late,
            "due_date": due_date,
            "scheduled_date": add_days(today, due_offset - 1),
            "assigned_to": technician,
            "assigned_by": "Administrator",
            "technician_notes": notes,
            **({"overall_result": result, "completion_date": add_days(today, due_offset + 1)}
               if result else {}),
        })
        doc.insert(ignore_permissions=True)
        created.append(doc.name)
        print(f"  Created PM WO: {doc.name} [{status}] asset={asset}")
...
def seed_repair_work_orders(assets: list[str], technician: str):
    ...
    specs = [
        ...
        (4, "Cannot Repair",  "Normal",    "Corrective",       48, 240, "Unknown",    1, 0),
    ]
    ...
    for a_idx, status, priority, r_type, sla_hours, hours_ago, root_cause, sla_breached, is_repeat in specs:
        ...
        completion_dt = base_dt - datetime.timedelta(hours=max(0, hours_ago - sla_hours)) \
            if status in ("Completed", "Cannot Repair") else None
        ...
        doc_data = {
            ...
            "assigned_to": technician,
            "assigned_by": "Administrator",
            "failure_description": f"Hỏng hóc thiết bị — {r_type} [{priority}]",
        }
        if root_cause:
            doc_data["root_cause_category"] = root_cause
            doc_data["repair_summary"] = f"Đã sửa chữa, nguyên nhân: {root_cause}"
            doc_data["dept_head_name"] = "Trưởng khoa xác nhận"
        if completion_dt:
            elapsed = (completion_dt - open_dt).total_seconds() / 3600
            doc_data["completion_datetime"] = completion_dt
            doc_data["mttr_hours"] = round(elapsed, 2)

        doc = frappe.get_doc(doc_data)
        doc.insert(ignore_permissions=True)
        created.append(doc.name)
        print(f"  Created Repair WO: {doc.name} [{status}] asset={asset}")
