...
| **D12.4 — markLabelPrinted-on-confirm** | `label_printed` CHỈ ghi SAU khi in xong: `markLabelPrinted(names)` qua **nút tường minh "Đã in xong"** (ưu tiên — chính xác) HOẶC `iframe.onafterprint`. **KHÔNG** ghi khi user mở hộp thoại rồi HUỶ (D8 audit-on-cancel). Batch: chỉ gửi name HỢP LỆ (loại item lỗi — parity luồng cũ FE6). | mở preview/print mà chưa confirm → `markLabelPrinted` KHÔNG gọi (mirror test cũ `AssetDetailView.qrPrint.test.ts:101-112`). |
...
| **D12.6 — gate quyền FE (parity route)** | thiếu cap `asset.print` → nút "In nhãn QR"/"In nhãn hàng loạt" **KHÔNG render** (`v-if can('asset.print')` — FE7, parity `AssetLabelPrint.meta.requiredCapabilities`). Lỗi 403 từ BE (D11) VẪN hiện toast VI (defense-in-depth). | mirror test cũ `AssetDetailView.qrPrint.test.ts:85-99` (no-print → nút absent). |
...
| **D20.3 — GỠ hàm legacy** | XOÁ `openLabelPreview` (@`:71-84`) + `confirmPrintLabel` (@`:86-100`). Đây là 2 hàm DUY NHẤT gọi `window.print()` (@`:88`) + đường ghi `label_printed` THỨ HAI (`confirmPrintLabel`→`markLabelPrinted`). | grep `frontend/src/views/asset/AssetDetailView.vue` **0 chuỗi `window.print(`** (lock bởi `AssetDetailView.qrPrint.test.ts:187-194`). |
...
**Lưu ý phạm vi (KHÔNG over-reach):** D20 CHỈ áp `AssetDetailView`. `AssetLabelPrintView` (batch) VẪN dùng `window.print()` + `LABEL_FORMATS` + `qr-label-sheet` — **ngoài phạm vi V24** (đường batch chưa được USER yêu cầu khai tử; `AssetLabelPrintView.format.test.ts` mount `AssetLabelPrintView` GIỮ XANH). Đây là lý do `LABEL_FORMATS`/`AssetQrLabel` KHÔNG bị xoá ở SSoT (D20.6).

**Test (CHỐT — đo được, vitest):**
- `AssetDetailView.qrPrint.test.ts` (đường PDF) **KHÔNG regress** — đặc biệt `:187-194` ("KHÔNG dùng window.print legacy") nay đúng tuyệt đối (grep 0 `window.print(` trong file). Các case PDF (gate nút, printLabels 1 lần, "Đã in xong"→markLabelPrinted, huỷ→không-ghi, preview iframe) GIỮ XANH.
- **Test MỚI (1 case tối thiểu):** khẳng định KHÔNG còn entry-point mở modal legacy — sau `mount(AssetDetailView)` + click "In nhãn QR" → KHÔNG render nút "In tem" và KHÔNG có `[data-testid="label-page-rule"]`/`showLabelModal` DOM (chỉ có `showPdfModal`). (Đặt trong `AssetDetailView.qrPrint.test.ts` hoặc file mới `assetDetailLabelLegacyGone.test.ts`.)
- Test legacy CŨ (nếu có case nào assert nút "In tem"/`showLabelModal`/`label-page-rule` cho `AssetDetailView`): **cập nhật/xoá cho khớp**. Khảo sát source xác nhận: KHÔNG test nào mount `AssetDetailView` assert legacy-label (chỉ `AssetLabelPrintView.format.test.ts` assert `labelFormatKey` nhưng mount `AssetLabelPrintView`, ngoài phạm vi) → kỳ vọng 0 test legacy cần sửa cho `AssetDetailView`; verify lại khi code.
...
| **QA-LEGACY-RETIRE (TDD vitest)** | **V24** | **D20 / test §** | `AssetDetailView.qrPrint.test.ts` 0 regression (đặc biệt `:187-194` no-`window.print` nay grep-0-tuyệt-đối). THÊM 1 case: sau `mount(AssetDetailView)` + click "In nhãn QR" → KHÔNG render nút "In tem" + KHÔNG có `[data-testid="label-page-rule"]`/DOM `showLabelModal` (chỉ `showPdfModal`). Cập nhật/xoá test legacy nào assert "In tem"/`showLabelModal`/`label-page-rule` cho `AssetDetailView` (khảo sát: kỳ vọng 0 — `AssetLabelPrintView.format.test.ts` mount `AssetLabelPrintView`, ngoài phạm vi; verify khi code). DoD: `grep -c 'window.print(' AssetDetailView.vue`==0 ∧ `grep -c 'markLabelPrinted(' AssetDetailView.vue`==1 ∧ `vue-tsc --noEmit` 0 lỗi (no dead-import) ∧ suite asset GREEN. **KHÔNG** git commit/bench restart/migrate. |
...
- Test V2 (suite hiện hành + mới): `AssetDetailView.qrPrint.test.ts` · `AssetLabelPrintView.test.ts` · `AssetLabelPrintView.format.test.ts` · `AssetListView.batchSelect.test.ts` · `frontend/src/api/tests/imm00.test.ts` (thêm test `printAssetLabelsPdf` + guard).
