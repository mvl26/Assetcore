...
- Guard bất biến: `frontend/src/router/uiAuditDocParity.test.ts` (khoá A1+A2+A5 — bảng không mục theo thời gian) · `frontend/src/router/uiFixPlanParity.test.ts` (khoá phủ-kín-route của phương án tổng — **0 route mồ côi**)
...
>   (`frontend/src/router/uiListShellLot1Parity.test.ts`: `view import ListPageShell` ⟺ ô «Lỗi+Thử lại» = ✅).
...
>   **từng ô** với bộ dò và được guard `uiListShellLot1Parity.test.ts` ép mỗi lượt chạy test.
...
>   Guard `frontend/src/router/uiDetailShellLot1Parity.test.ts` bám cột «Trạng thái» của **sổ lô** chứ không bám
...
```
Delta `+1 file / +15 test` = đúng bằng guard mới `uiAuditDocParity.test.ts`; **số file đỏ không đổi (1)**
...
Guard riêng (A6): `npx vitest run src/router/uiAuditDocParity.test.ts`
```
...
**File ĐỎ (1):**
1. `frontend/src/views/dashboard/personas/personaDashboards.test.ts` — `D-FE-1: current=opsmgr → render OpsmgrDashboardView`, dòng 59:
...
Guard mới của vòng này (A6): `npx vitest run src/router/uiAuditDocParity.test.ts` → xem §8.
...
| **AC-UX-023** | `frontend/src/views/dashboard/personas/personaDashboards.test.ts:59` | **Test ĐỎ**: kỳ vọng chuỗi «Trưởng phòng VT-TTBYT» đã bị commit `44cbff9` gỡ khỏi tiêu đề; test chưa cập nhật | P1 | 2 | §4 |
...
| **AC-UX-066** | `frontend/src/components/common/bareConfirmBudget.test.ts` (MỚI) | **Không có guard cho nợ `confirm()`**: allowlist theo tên file (khuôn AC-UX-055) mù với hồi quy **bên trong** file đã có tên; và trần treo lơ lửng khiến doc trôi khỏi đĩa (số «44/31» sống sót qua 2 vòng). Ngân sách theo cặp **(file, số lần)**, CHỈ-GIẢM **hai chiều** — giảm mà quên hạ sổ cũng ĐỎ (ADR-UX-18) | P1 | 7 | §6 của `06` |
...
`frontend/src/router/uiAuditDocParity.test.ts` — khoá 3 bất biến, chạy trong bộ FE bình thường:
...
- **Decision** — 4 điều, thi hành nguyên khối ở vòng 2:
  ...
  4. **Guard 2 chiều là điều kiện tồn tại của token**: `src/design/tokens.parity.test.ts` chặn cả token mồ côi lẫn var mồ côi;
     `ui/uiPrimitiveHygiene.test.ts` chặn palette thô / thiếu test co-located / chuỗi EN.
...
- **Consequences**:
  ...
  - Shell **dumb tuyệt đối**: cấm import `vue-router` / `@/stores` / `@/api` / `@/components/common` — `ProcurementPlanCreate.test.ts`
...
- **Decision** — 4 điều:
  1. **Khuôn nằm ở tier-1** `frontend/src/components/common/DetailPageShell.vue`, **không** thêm vào `components/ui/`.
     ...
     cấm import ngược; (ii) guard `uiPrimitiveHygiene.test.ts` khoá 3 vế *(số export == số `.vue` == số test)* ở **8** — file thứ 9
...
- **Consequences**:
  - `DetailPageShell` **không** chịu guard vệ sinh tầng 0 (`INV-UI-*` chỉ quét `components/ui/`) ⇒ phải tự khoá bằng
    `DetailPageShell.test.ts` + 3 lệnh `grep -c` trong `03 §6.3` (no-fork: `DetailLoadError` ≥1, `DetailTabBar` ≥1, nhãn nút nạp lại **= 0**).
...
- **Consequences**: `EXPECTED_PRIMITIVES` giữ **8**, `ui/uiPrimitiveHygiene.test.ts` **không đổi**. Auto-focus khi mở làm `document.activeElement` đổi trong mọi test cũ có mount view chứa modal ⇒ phải chạy **toàn** suite trước khi khai xong. Đổi bất kỳ prop/slot/emit/`data-testid`/chuỗi class nào của `BaseModal` từ nay là **breaking change** cho 19 file.
