...
| **INV-SLA-5** (badge FE không lệch tile) | Incident currently-overdue-open, cờ DB còn 0 | BE enrich `is_response_breached`/`is_resolution_breached` (derive live) trên mỗi row `list_incidents` + `active_incidents`; FE đọc field derived. **Số badge live == tile** |
| **INV-SLA-6** (terminal exclude) | Incident `Cancelled`/`Closed`/`Resolved` đã đóng đúng hạn (cờ=0) | KHÔNG bị tính `live-overdue` (chỉ vào count qua nhánh `cờ=1` nếu đã từng breach) — không phantom-count thiết bị đã đóng |
