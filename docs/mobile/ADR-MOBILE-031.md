...
> **CONTRACT-ONLY** — TUYỆT ĐỐI KHÔNG đụng `api/imm11.py`/`services/imm11.py` (`send_to_lab` @169/@1270 ĐÃ LIVE trên đĩa). Không reload/migrate/commit. **BE grep-verify baseline @source TRƯỚC bump** (571/62/51/714/740 = grounded 2026-07-11 sau ADR-030; nếu phiên khác đã dịch → tính lại delta +1 path / +10 test / +1 c5 / +10 suite-sum / +10 oas-total). DoD: `bench --site miyano run-tests --app assetcore --module assetcore.tests.guards.test_mobile_oas` = **'Ran N OK' THẬT** (N=count mới) · `.test_mobile_docset` = **Ran 9 OK** · `test_mobile_security_gate` GREEN · `test_oas_d12/d15/d17` UNCHANGED (đọc dòng cuối, KHÔNG false-green).
