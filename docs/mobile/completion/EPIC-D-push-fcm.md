...
- **Acceptance:**
  - `[AUTO]` `bench --site miyano run-tests --module assetcore.tests.test_mobile_oas` (2 path rời `_STUB_PATHS`; requestBody/response typed; 0 dangling `$ref`). ✅ **DONE+GREEN (Vòng 17 — 131 OK).**
...
- **Files (Modify):** `assetcore/services/notifications.py` (`_dispatch` chèn kênh #3 + helper `_push_event_route` — **KHÔNG sửa call-site, KHÔNG đổi chữ-ký `_dispatch`**) · `assetcore/api/mobile/v1/device_token.py` (thêm `@rate_limit` register). **(Create):** TC D6 trong `assetcore/tests/test_mobile_device_token.py` (class `TestMobilePushDispatch`).
- **Acceptance:**
  - `[AUTO test]` `bench --site miyano run-tests --module assetcore.tests.test_mobile_device_token` (TC D6: `_dispatch` mock-FCM bắn push CHỈ tới token `enabled=1` của recipient; token `enabled=0` bị bỏ; FCM raise → in-app/email VẪN gửi (fail-safe) — assert side-effect THẬT, KHÔNG chỉ `return` (chống false-green LL-TEST-21)).
  ...
  - `[AUTO regression]` `bench --site miyano run-tests --module assetcore.tests.test_notification_framework && ... test_notifications && ... test_imm12_notify` (kênh 1+2 KHÔNG đổi hành vi — 0 regression).
