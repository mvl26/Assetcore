...
>
> Hợp đồng máy-đọc: OpenAPI component **`OAuthError400`** (union 2 shape; `error` = key chung required) wire `'400'`→`OAuthError400` **CHỈ** lên `getOAuthToken`; `revokeOAuthToken` 200 = empty object. `OAuthError400` **KHÁC** Error envelope + **KHÁC** `FrappeRawError` của business pre-handler (401/403/429). Guard = `assetcore/tests/test_mobile_oas.py` (`TC-MOB-OAUTH-TOKEN-01..05`). Bảng error 2-lớp: [`04-api-contract.md §5b`](./04-api-contract.md).
...
- **Guard endpoint↔capability (A14 — củng cố invariant §3/§3.2):** `assetcore/tests/test_mobile_capability_map.py` (`TC-MOB-CAP-01..06`) — 10 endpoint MVP ↔ matrix [`11-phase-a-exit.md §1`](./11-phase-a-exit.md) ↔ SSoT `rbac.py`; binding `(DocType, ptype)` + anti-cap-creep (`len==97`) + version `v97.c30c69b8974d` đóng băng (drift = test ĐỎ).
