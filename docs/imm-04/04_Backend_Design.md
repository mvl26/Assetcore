...
Sau đó **BẮT BUỘC** `python scripts/gen_fe_messages.py` (AST-parse `utils/messages.py` → `frontend/src/i18n/messages.ts` + `messages.types.ts`, **AUTO-GENERATED — cấm sửa tay**). Bỏ bước này ⇒ FE rơi về toast `SYS-500` "liên hệ IT" (class-of-bug đã ghi ở STATE Blocker#1). Lượt regen này cũng đồng thời đóng backlog `IMM09-SELF-INSPECT-FORBIDDEN` nếu code đó đã có trong `messages.py`.
