...
> **Trạng thái**: Module IMM-16 đã LIVE (Wave 2). Suite test hiện tại: `assetcore/tests/imm16/test_imm16.py` — 12 TestCase, 29 test method (xem III.2). Các dòng đánh dấu `⬜ Planned` là backlog mở rộng chưa viết.
...
File: `assetcore/tests/imm16/test_imm16.py` — **12 TestCase, 29 test method** (LIVE). Mỗi test class trace về ≥ 1 dòng I.1.
...
| **AA-16-17 (auto-Finding THẬT, CR-27d — RED→GREEN)** | `complete_audit_checklist` với 1 `Major NC` | `findings_created==1` · `status==Reporting` · `notes` persist · `IMM Audit Trail` +1 (`audit_checklist_completed`). **RED-before:** trước fix (rule từ field-ảo → MandatoryError nuốt) → `findings_created==0` (test cũ pin ==0 @test_imm16.py:531); sau fix → 1 |
| **AA-16-18 (persist THẬT, CR-27d)** | sau AA-16-17, query `IMM Compliance Finding` theo `source_record_doctype="IMM Internal Audit"` + `source_record=<audit>` | ĐÚNG 1 row; `severity=="High"`; `rule` KHÔNG rỗng (== `AUDIT-INTERNAL-NC`); `detected_date` + `evaluation_date` KHÔNG rỗng. Chứng minh persist THẬT ≠ `len(payload)` |
| **AA-16-19 (2 NC → 2 Finding, CR-27d)** | `complete_audit_checklist([{idx:1,Major NC},{idx:2,Minor NC}])` | `findings_created==2`; query source_record ⇒ 2 row, `severity` = `{High, Medium}` (Major→High, Minor→Medium). **KHÔNG dedup** (2 NC cùng audit/ngày KHÔNG gộp 1) |
| **AA-16-20 (mixed payload, CR-27d)** | `complete_audit_checklist([Compliant, Major NC, Minor NC])` | `findings_created==2` — Compliant/N/A KHÔNG sinh Finding |
| **AA-16-21 (canonical rule idempotent, CR-27d)** | 2 audit khác nhau, mỗi cái 1 Major NC → complete cả 2 | `frappe.db.count("IMM Compliance Rule", {"rule_code":"AUDIT-INTERNAL-NC"})==1` (không nhân bản rule fallback) |
| **AA-16-22 (fail-loud, CR-27d)** | monkeypatch `ComplianceFindingRepo.create` raise error THẬT, gọi complete với 1 Major NC | raise (KHÔNG nuốt); audit KHÔNG commit sang Reporting; KHÔNG return `findings_created` > 0 (hết success-giả) |
...
| Backend test records | tạo trong `setUpClass` (`TestImm16Base`) với prefix `_Test` | `assetcore/tests/imm16/test_imm16.py` |
...
```bash
# Module test (LIVE)
bench --site [site] run-tests --app assetcore --module assetcore.tests.imm16.test_imm16
# Coverage
coverage run -m unittest assetcore.tests.imm16.test_imm16 && coverage report
# Workflow smoke
...
- [x] CI command chạy clean (`bench run-tests --module assetcore.tests.imm16.test_imm16`)
