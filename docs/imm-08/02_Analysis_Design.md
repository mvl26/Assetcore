...
| BR-08-08 | **Cổng hoàn thành checklist — 2 chế độ lỗi PHÂN BIỆT** (khi WO chuyển `Completed`/`Halted–Major Failure`): (a) **bảng kiểm RỖNG** — 0 dòng `checklist_results` → `IMM08_CHECKLIST_EMPTY` (BR-08-19, bịt lỗ vacuous-pass, đây mới là cổng chống nghiệm-thu-giả); (b) **≥1 dòng nhưng có dòng thiếu `result`** → `IMM08_CHECKLIST_INCOMPLETE`. Hai mã KHÔNG trộn (empty ≠ incomplete — root-cause + action-hint khác nhau, xem ADR-IMM08-CHECKLIST-EMPTY-01). | `validate_work_order()` — guard rỗng TRƯỚC vòng lặp thiếu-result | TC-PM-02, TC-PM-EMPTY-01..02 |
...
| BR-08-19 | **Chống nghiệm-thu-giả PM khi bảng kiểm RỖNG — bịt lỗ vacuous-pass BR-08-08 (Vòng 3)** — Cổng BR-08-08 hiện lặp `for item in (doc.checklist_results or [])`; khi WO có **0 dòng** `checklist_results` (tạo template-less / template 0 mục / BR-08-01 rò lọt) vòng lặp **vacuous-pass** ⇒ WO chuyển `Completed` + sinh **PM Task Log KHÔNG có bằng chứng công việc** (vi phạm CLAUDE.md §5 "mọi nghiệp vụ phải có record" + NĐ98 hồ sơ bảo trì). **Fix**: trong `validate_work_order`, khi `doc.status ∈ {Completed, Halted–Major Failure}` thêm **guard TRƯỚC vòng lặp** — `if not (doc.checklist_results or []): nthrow_in_hook(MSG.IMM08_CHECKLIST_EMPTY)`. (a) Mã MỚI `IMM08-CHECKLIST-EMPTY` (≠ `IMM08-CHECKLIST-INCOMPLETE`) — message VI: *"Không thể hoàn thành PM: bảng kiểm chưa có mục nào (thiếu bảng kiểm mẫu) — vui lòng gắn bảng kiểm trước khi nghiệm thu."* http_status 422, severity warning. (b) `submit_result` KHÔNG cần sửa cho nhánh này: `wo.save()` chạy validate hook → `frappe.ValidationError` → `except` line 1043 wrap `ServiceError(VALIDATION)` (message_code đã stash `frappe.local.response`). **WO.status GIỮ nguyên** (save rollback), **docstatus=0** (`wo.submit()` KHÔNG chạm). (c) **Persist-thật**: KHÔNG PM Task Log mới, `AC Asset.next_pm_date/last_pm_date` KHÔNG đổi, KHÔNG advance PM Schedule (mọi side-effect nằm trong `handle_work_order_submit` = post-submit, KHÔNG chạm). (d) **No-regression**: WO ≥1 dòng thiếu-result → vẫn `IMM08_CHECKLIST_INCOMPLETE` (BR-08-08 cũ, guard rỗng KHÔNG nuốt). **Boundaries — Always**: guard chỉ so 0-dòng (KHÔNG so nội dung); precedence EMPTY > INCOMPLETE (0-dòng đánh EMPTY, ≥1-dòng mới xét INCOMPLETE). **Never**: raise→HTTP-4xx (phải nthrow_in_hook→ValidationError→ServiceError Decision-B); reuse `{item}` interpolation của INCOMPLETE (không có item để nêu tên); áp side-effect trước khi gate pass. | `validate_work_order()` guard rỗng; `MSG.IMM08_CHECKLIST_EMPTY` (messages.py) | TC-PM-EMPTY-01 (RED→GREEN raise+status giữ), TC-PM-EMPTY-02 (persist DB: 0 task-log/date bất biến) |
| BR-08-20 | **Chống drop âm-thầm kết quả checklist khi idx payload lệch (AC5 — OPTIONAL BE-2, khuyến nghị làm)** — `submit_result` build `result_map = {r["idx"]: r for r in checklist_results if "idx" in r}` rồi CHỈ áp cho `row.idx in result_map`. Nếu payload mang `idx` KHÔNG tồn tại trong `wo.checklist_results` → dòng đó **rơi câm** (KTV tưởng đã ghi nhưng server bỏ) = mất bằng chứng, vi phạm audit-trail. **Fix (khi WO có ≥1 dòng)**: tính `applied = len([row for row in wo.checklist_results if row.idx in result_map])`; nếu `len(result_map) > applied` (có idx payload không khớp dòng nào) → `nthrow(MSG.IMM08_CHECKLIST_IDX_UNKNOWN, idx=<idx lệch>)`. **Precedence**: EMPTY (0-dòng, BR-08-19) > IDX_UNKNOWN (≥1-dòng + idx lệch) > INCOMPLETE (≥1-dòng + idx khớp + dòng thiếu result). Guard IDX **bỏ qua khi `wo.checklist_results` rỗng** (nhường cổng EMPTY sở hữu case template-less). `applied count` = số idx khớp thực ⇒ subset hợp lệ (payload ít hơn dòng) KHÔNG kích IDX, để INCOMPLETE bắt dòng chưa rate. **Boundaries — Always**: chỉ raise khi có idx KHÔNG khớp DÒNG NÀO (drift thật); subset đúng-idx → im lặng cho INCOMPLETE xử. **Never**: coi drift là success câm; đổi shape payload success 5-key. | `submit_result()` idx-drift guard; `MSG.IMM08_CHECKLIST_IDX_UNKNOWN` | TC-PM-IDX-01 (RED payload idx-lạ → raise, applied==khớp-thực) |
| BR-08-18 | **Idempotency `submit_pm_result` cho mobile write-outbox (CR-24-PM)** — param optional `client_request_id: str = ""` (khoá outbox bền, = `item.id` UUID mint-1-lần, ổn định qua mọi re-drain). Cơ chế **KHÔNG DocField mới, KHÔNG `bench migrate`**: idempotency neo trên **trạng thái TERMINAL của chính PM Work Order** (natural key = `wo.name`; "biên nhận" bền = `docstatus==1` + `status=Completed` + `completion_date` đã persist + Corrective WO đã link qua `source_pm_wo`). (a) **`client_request_id=""` (rỗng/absent) ⇒ KHÔNG dedup — hành vi legacy byte-identical**: submit lần 2 lên WO `docstatus==1` vẫn `nthrow(MSG.IMM08_ALREADY_SUBMITTED)` (in-handler HTTP-200 Error envelope). NULL-semantics. (b) **`client_request_id` non-empty ⇒ idempotent replay**: nhánh already-submitted CHUYỂN từ raise → **re-read state terminal → dựng lại ĐÚNG payload 5-key** `{name,new_status,is_late,next_pm_date,cm_wo_created}` (byte-for-byte == lần 1) → return success, **KHÔNG áp lại side-effect**. `completion_date` KHÔNG drift (đọc lại field đã persist; `next_pm_date` re-derive deterministic từ nó ⇒ cùng giá trị — BR-08-03); `cm_wo_created` re-đọc qua `find_one(source_pm_wo, Corrective)` ⇒ **CM WO count KHÔNG tăng lần 2**. (c) **Race** (2 request gần-đồng-thời CÙNG key trên WO `docstatus==0`): đúng **1 winner** commit `wo.submit()`; request thua bắt exception va-chạm (stale-doc/`DocstatusTransitionError`/duplicate) → convert sang re-read terminal → trả payload winner. **KHÔNG rò `IMM08_ALREADY_SUBMITTED`/'must be unique'** ra caller. (d) **Scope theo natural key** `(wo_name, client_request_id)`: 2 WO khác nhau + 2 key khác nhau ⇒ độc lập, KHÔNG nhiễm chéo. **Bất biến giữ nguyên**: `rbac.require("pm.submit")` + `@frappe.whitelist(methods=["POST"])` + anti-spoof (signature KHÔNG nhận `user`) + envelope Decision-B (`handle`/`_ok`). **Payload KHÔNG đổi shape** (khoá bởi OAS closed `PmSubmitResultResponse` required-EXACT-5 + FE type `api/imm08.ts:131` + guard `test_mob_oas_submitpm_f/_i`). Xem **ADR-IMM08-IDEMPOTENCY-01** (04 §4) — vì sao KHÁC CR-24 (report_incident dùng unique DocField + migrate). | `api/imm08.py::submit_pm_result` (+param); `services/imm08.py::submit_result` (gate replay + race-catch); OAS `SubmitPmResultRequest` (+prop) + guard `test_mobile_oas` `_SUBMIT_PM_RESULT_REQUEST_PROPS` 6→7 — **land ATOMIC (BE-owned)** | TC-PM-IDEM-01..06 (RED-before/GREEN-after) |
