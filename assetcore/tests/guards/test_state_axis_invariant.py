...
Run:
  bench --site miyano run-tests --app assetcore \
      --module assetcore.tests.guards.test_state_axis_invariant
"""
...
def _active_workflows() -> list[dict]:
    ...
    # Chốt dân số (§5.2 N5): thư mục workflow bị dời ⇒ NÉM LỖI, không trả [] rồi PASS.
    workflows = [_load_json(p) for p in list_files(WORKFLOW_DIR, ".json", min_count=18, recursive=False)]
...
def _service_files() -> list[str]:
    ...
    return list_files(svc, ".py", min_count=20, recursive=False)
