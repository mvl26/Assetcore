...
Run: bench --site miyano run-tests --app assetcore --module assetcore.tests.guards.test_workflows
"""
...
class TestWorkflowsRegistered(FrappeTestCase):
...
class TestWorkflowDocstatusValidity(FrappeTestCase):
...
class TestWorkflowRolesExist(FrappeTestCase):
...
class TestIMM12IncidentRCAGate(FrappeTestCase):
...
class TestIMM11CapaTransition(FrappeTestCase):
...
class TestWorkflowAdminOverrideInvariant(FrappeTestCase):
