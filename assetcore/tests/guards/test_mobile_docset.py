...
Run: bench --site miyano run-tests --module assetcore.tests.guards.test_mobile_docset
"""
...
class TestMobileDocsetIntegrity(unittest.TestCase):
    ...
    def test_tc_mob_doc_02_adr_and_contract_registration(self) -> None:
        ...
        # CR-29-write — sub-assertion (KHÔNG là `def test` riêng — giữ docset ĐÚNG 9 `def test`,
        #   precedent F-V1 doc_03): 04-api-contract §8.58 nhắc đủ 3 WRITE opId IMM-15 allocation +
        #   trỏ READ pair đã mirror (đóng [FLAG-OWNER] doc-gap round 3). STDLIB-only (read-text substring).
        self._assert_alloc_write_ops_documented()

    def _assert_alloc_write_ops_documented(self) -> None:
        """CR-29-write — 04-api-contract.md §8.58 documents 3 IMM-15 WRITE opId + trỏ READ pair.

        Sub-assertion gọi từ TC-MOB-DOC-02 (KHÔNG là `def test` riêng → giữ docset ĐÚNG 9 `def
        test` ⇒ guard-suite/mobile-OAS counter KHÔNG đẩy; precedent `_assert_handoff_curl_path_
        matches_yaml_ssot` gọi từ TC-03). Đóng [FLAG-OWNER] doc-gap round 3: READ pair
        listAllocations/getAllocation đã mirror OAS+guard NHƯNG thiếu §8.xx doc — nay §8.58 phủ
        cả WRITE 3-op (approve/issue/cancel) + trỏ READ pair. STDLIB-only (giữ kỷ luật TC-05).
        """
        contract_md = _MOBILE_DIR / "04-api-contract.md"
        self.assertTrue(contract_md.is_file(), "Thiếu docs/mobile/04-api-contract.md.")
        txt = contract_md.read_text(encoding="utf-8")
        self.assertIn("8.58", txt, "04-api-contract THIẾU section §8.58 (CR-29-write IMM-15 WRITE-family).")
        for opid in ("approveAllocation", "issueAllocation", "cancelAllocation"):
            self.assertIn(opid, txt, f"§8.58 THIẾU WRITE opId `{opid}` (họ IMM-15 F9 WRITE 3-op).")
        for read_opid in ("listAllocations", "getAllocation"):
            self.assertIn(read_opid, txt, f"§8.58 PHẢI trỏ READ pair `{read_opid}` (đóng doc-gap round 3).")
...
class TestMobileGuardSuiteCountParity(unittest.TestCase):
    ...
    def test_tc_mob_doc_09_transition_baseline_off_by_delta(self) -> None:
        ...
        pm_calendar_delta = 2  # CR-62d MỞ-NHÁNH IMM-08 getPmCalendar mine-scope curate (mobile Spec 62 "Lịch PM tháng", 2026-07-24): +2 TC test_mobile_oas.py TestMobileGetPmCalendarContract (operation_present + mine_param_parity — GET-read lịch PM tháng {month,events[],summary}; toggle "Chỉ việc của tôi" mine=1 → assigned_to==session.user server-resolve; 3 schema CLOSED PmCalendar/PmCalendarEvent[is_late integer enum[0,1] Check-emit-int CR-62b]/PmCalendarEnvelope; 5 param year/month(req)+asset_ref/technician/mine(opt int enum[0,1] default 0); 200 oneOf [Env,Error] Decision-B; ∈ _MVP_READ_ENVELOPE path/opId 104→105 c5 93→94; CONTRACT-ONLY pure-yaml 0 .py-prod-mới [BE mine=0 additive @api/imm08.py:170]). Trừ delta này để giữ pre_fc3_six==191 (transition-baseline post-F-C3 KHÔNG đổi).
        ...
        pending_approvals_inbox_delta = 7  # APPROVAL-INBOX-CR32 getPendingApprovalsInbox (imm00.get_pending_approvals_inbox, 2026-07-16): +7 TC TestMobilePendingApprovalsInboxContract a..g (inbox gộp "Phiếu chờ tôi duyệt" XUYÊN MODULE imm04+imm00+imm15; path 90→91; 2 schema PendingApprovalItem/PendingApprovalsInboxEnvelope; tag account GIỮ 15; BE LIVE MỚI api/imm00.py — runtime guard tests/test_imm00_approvals_inbox.py).
        ...
        attach_photo_idempotency_delta = 3  # IDEMPOTENCY-PHOTO-CR24 phần dư attach_incident_photo client_request_id (BR-12-26/ADR-IMM12-10, 2026-07-16): +3 TC TestMobileAttachPhotoIdempotencyContract a..c (AttachIncidentPhotoRequest +prop optional client_request_id; closed GIỮ + required EXACT 2 GIỮ + path GIỮ 91; dedupe composite {incident}::{key} trên Custom Field File.ac_client_request_id unique NULL-store; land ATOMIC cùng handler api/imm12.py + service services/imm12.py)
        label_print_delta = 8  # CR-35 QR-LABEL-PRINT (IMM-00) mirror-parity getAssetLabelData+markLabelPrinted (imm00.get_asset_label_data + imm00.mark_label_printed, 2026-07-18): +8 TC test_mobile_oas.py TestMobileLabelPrintContract label_a..h (2 endpoint QR in-tem đã LIVE wire RAW; getAssetLabelData GET-read OBJECT 8-field tem 1-asset [regression-guard CR-35 KHÔNG type:array] + markLabelPrinted POST write-audit {printed,event_count}; @rate_limit @imm00.py:722/815 → declare 429 + rbac.require asset.print dispatcher-403 + guest 401 ⇒ {200,401,403,429}; 5 schema CLOSED AssetLabelData[8 field AST drift-proof build_asset_label_data return-dict]/AssetLabelDataEnvelope/MarkLabelPrintedRequest[{assets:string JSON}]/MarkLabelPrintedData[{printed:array<string>,event_count:integer}]/MarkLabelPrintedEnvelope; ∈ _MVP_READ_ENVELOPE[getAssetLabelData]/_MVP_ACTION_ENVELOPE[markLabelPrinted] + _MVP_BUSINESS_PATHS + _PATHS_REQUIRE_429/_RATE_LIMIT_SOURCE_MAP; path/opId 92→94 c5 81→83; CONTRACT-ONLY pure-yaml 0 .py — endpoint đã LIVE mirror-only). Trừ delta này để giữ pre_fc3_six==191.
        recall_fsca_delta = 8  # CR-26 IMM-10 RECALL/FSCA mirror-parity checkAssetRecall (imm10.check_asset_recall, 2026-07-18): +8 TC test_mobile_oas.py TestMobileRecallContract recall_a..h (GET-read cảnh-báo thu-hồi/FSCA màn quét QR — CÙNG-HỌ getAssetScanInfo QR-scan/token/rate-limit; path GET assetcore.api.imm10.check_asset_recall opId checkAssetRecall tag asset; @rate_limit 30/60s/IP bucket RIÊNG @imm10.py:46 → declare 429 + rbac.require asset.read dispatcher-403 + guest 401 ⇒ {200,401,403,429}; 3 schema CLOSED RecallNoticeItem[8 field = services.imm10._RECALL_ROW_FIELDS DRIFT-PROOF import-source, published_date string/date còn lại string]/RecallCheckResult[{asset,has_recall boolean,recalls[]→RecallNoticeItem}]/RecallCheckEnvelope[{success enum[true],data}]; 200 oneOf [RecallCheckEnvelope,Error] Decision-B — 404 leak-safe @:85 + vendor-IDOR-403 @:90 arrive HTTP-200+Error; ∈ _MVP_READ_ENVELOPE + _MVP_BUSINESS_PATHS + _PATHS_REQUIRE_429/_RATE_LIMIT_SOURCE_MAP; path/opId 91→92 c5 80→81; CONTRACT-ONLY pure-yaml 0 .py — endpoint đã build+green 9 TC b1-b9). Trừ delta này để giữ pre_fc3_six==191 (transition-baseline post-F-C3 KHÔNG đổi).
        audit_write_delta = 12  # CR-27a MỞ-NHÁNH-IMM16-F7-WRITE startAudit+completeAuditChecklist (imm16.start_audit + imm16.complete_audit_checklist, 2026-07-18): +12 TC test_mobile_oas.py TestMobileInternalAuditContract audit_a..l (2 POST-action WRITE-family — get_audit ĐÃ curate opId getInternalAudit [GIỮ, KHÔNG rename Hyrum] ⇒ family IMM-16 audit ĐỦ 4 op list+get+start+complete; path POST assetcore.api.imm16.start_audit opId startAudit + assetcore.api.imm16.complete_audit_checklist opId completeAuditChecklist tag compliance; 7 schema CLOSED StartAuditRequest/StartAuditResponse[3-key {name,status,actual_start} status enum 4 AuditStatus]/StartAuditEnvelope/AuditChecklistVerdictInput[element-shape items JSON-string, finding_status enum=set(_FINDING_STATUS_TO_RESULT.keys()) drift-proof import-source → _RESERVED_ORPHANS]/CompleteAuditChecklistRequest[{audit_name,items:string JSON} closed]/CompleteAuditChecklistResponse[4-key {audit_name,items_count,findings_created,status} items_count/findings_created integer GENUINE KHÔNG enum[0,1]]/CompleteAuditChecklistEnvelope; 200 oneOf [Env,Error] Decision-B route-by-VALUE NOT_FOUND/BAD_STATE/INVALID_PARAMS/cap-403 → HTTP-200 Error; 403 SINGLE Forbidden dispatcher-403 guest mirror startRepair; ∈ _MVP_ACTION_ENVELOPE + _MVP_BUSINESS_PATHS; CR-27b verdict finding_status→child.result round-trip AC7-known-gap RESOLVED; path/opId 94→96 c5 83→85; CONTRACT-ONLY pure-yaml 0 .py — 2 endpoint LIVE mirror-only). Trừ delta này để giữ pre_fc3_six==191 (transition-baseline post-F-C3 KHÔNG đổi).
        needs_read_family_delta = 7  # CR-33-read MỞ-NHÁNH-IMM01 NEEDS-REQUEST READ-family listNeedsRequests+getNeedsRequest (imm01 2 GET, 2026-07-19): +7 TC test_mobile_oas.py TestMobileNeedsRequestContract (list+detail "Mở Nhu cầu thiết bị"; path/opId 99→101 c5 88→90; rows-key data.items[] variant #2 FLAT pagination @source api/imm01.py:122; CONTRACT-ONLY pure-yaml 0 .py — 2 endpoint LIVE mirror-only). Trừ delta này để giữ pre_fc3_six==191 (transition-baseline post-F-C3 KHÔNG đổi).
        alloc_write_delta = 7  # CR-29-write MỞ-NHÁNH-IMM15-F9-WRITE approve/issue/cancel_allocation (imm15 3 POST-action, 2026-07-18): +7 TC test_mobile_oas.py TestMobileAllocationWriteContract alloc_write_a..g (đóng slice WRITE họ IMM-15 "Xuất kho phụ tùng phục vụ WO" — READ pair listAllocations R40/getAllocation R42 đã curate; 3 path POST assetcore.api.imm15.{approve,issue,cancel}_allocation opId approveAllocation/issueAllocation/cancelAllocation tag inventory; 7 schema CLOSED AllocationActionRequest[{allocation,name} req[allocation] dual json+form]/ApproveAllocationResponse[2-key {name,workflow_state}]/IssueAllocationResponse[3-key {name,workflow_state,stock_movement_ref} GROUNDED]/CancelAllocationResponse[2-key]/Approve|Issue|CancelAllocationEnvelope; return-dict AST drift-proof @services/imm15.py:289/313/444; 200 oneOf [Env,Error] Decision-B NOT_FOUND/BAD_STATE/cap-403 → HTTP-200 Error; state-gate: approve Requested-only / issue LOẠI Picked IRREVERSIBLE / cancel reject Issued,Returned; cap inventory.submit(approve)/inventory.write(issue,cancel); path/opId 96→99 c5 85→88; CONTRACT-ONLY pure-yaml 0 .py — 3 endpoint LIVE mirror-only). Trừ delta này để giữ pre_fc3_six==191 (transition-baseline post-F-C3 KHÔNG đổi).
        needs_write_family_delta = 7  # CR-33-write MỞ-NHÁNH-IMM01-WRITE create_needs_request+get_allowed_transitions+transition_workflow (imm01 2 POST + 1 GET, 2026-07-19): +7 TC test_mobile_oas.py TestMobileNeedsRequestContract write_paths_present/write_200_oneof_envelopes/write_request_bodies/write_response_parity_closed_keys/transitions_item_is_object_not_flatstring/write_membership_and_no_orphan/write_endpoints_whitelisted_live (đóng nhánh WRITE DEFER ở CR-33-read; 3 endpoint LIVE @api/imm01.py:275/248/316; 7 schema pre-staged @yaml 11166-11409 chỉ $ref [5 orphan Request/Envelope ĐÃ wire → hết orphan, KHÔNG đụng _RESERVED_ORPHANS]; create/transition ∈ _MVP_ACTION_ENVELOPE + get_allowed_transitions ∈ _MVP_READ_ENVELOPE + 3 ∈ _MVP_BUSINESS_PATHS; response parity CLOSED exact-key == AST return-dict BE [create {name,workflow_state} / transitions {workflow_state,transitions} / transition {name,workflow_state,docstatus}]; transitions[] OBJECT {action,next_state} KHÁC allowed_transitions[] flat-string; ⚠️ docstatus==0 sau 'Gửi đề xuất' [Submitted-state doc_status="0" @workflow.json:23, KHÔNG 1 — verified probe]; path/opId 101→104 c5 90→93; CONTRACT-ONLY pure-yaml 0 .py). Trừ delta này để giữ pre_fc3_six==191 (transition-baseline post-F-C3 KHÔNG đổi).
        warranty_detail_parity_delta = 5  # CR-38 AssetDetail warranty parity (IMM-00, 2026-07-19): +5 TC test_mobile_oas.py TestMobileAssetDetailWarrantyParity adwarranty_a..e (getAsset enrich 2 field warranty_expired[boolean server-flag _is_warranty_expired]+warranty_expiry_date[string date nullable _date_str_or_none] ĐỐI XỨNG build_asset_scan_info/AssetScanInfo; schema-FIELD add trên AssetDetail open additionalProperties:true GIỮ; path/opId/whitelist 0 mới ⇒ oas_baseline BASELINE_TOTAL GIỮ; enrich api/imm00.py get_asset — cần USER reload gunicorn --preload). Trừ delta này để giữ pre_fc3_six==191 (transition-baseline post-F-C3 KHÔNG đổi).
        available_actions_incdetail_delta = 6  # CR-39 IMM-12 IncidentDetail available_actions[] server-driven 6 CTA (2026-07-23): +6 TC test_mobile_oas.py TestMobileIncidentDetailAvailableActionsParity incact_a..f (get_incident_detail bồi available_actions[] TÁI DÙNG $ref AvailableAction [6 CTA acknowledge/start_work/resolve/close/reopen/cancel, enabled=transition∩cap∩business_gate BR-12-02, route='' CTA-in-screen]; schema-FIELD add trên IncidentDetail open additionalProperties:true GIỮ; property_count 43→44 renamed sla_f KHÔNG +TC; path/opId/whitelist 0 mới ⇒ oas_baseline BASELINE_TOTAL GIỮ; CONTRACT-ONLY pure-yaml 0 .py — BE _build_incident_actions @Bước-4). Trừ delta này để giữ pre_fc3_six==191 (transition-baseline post-F-C3 KHÔNG đổi).
        enrich_incdetail_delta = 5  # CR-40 IMM-12 (2026-07-24): +5 TC test_mobile_oas.py TestMobileIncidentDetailEnrichParity incenrich_a..e (get_incident_detail bồi 3 field enrich reporter_name/assigned_to_name/asset_lifecycle_status; property_count 44→47; schema MỞ additionalProperties:true GIỮ; path/opId/whitelist 0 mới ⇒ oas_baseline GIỮ; CONTRACT-ONLY pure-yaml 0 .py — BE _enrich_asset_names+get_incident_detail @Bước-4). Trừ delta này để giữ pre_fc3_six==191 (transition-baseline post-F-C3 KHÔNG đổi).
        cr74_detail_readgate_delta = 7  # CR-74 IMM-08/09/11/12 (2026-07-25): +6 TC test_mobile_oas.py TestMobileDetailReadGate cr74_a..f (cụm C6-DETAIL — 4 GET-detail getPmWorkOrder/getRepairWorkOrder/getIncident/getCalibration khai nhánh 403 IN-ENVELOPE: FORBIDDEN arrive HTTP-200, client route-by-VALUE body.success, HIỂN THỊ MESSAGE + KHÔNG LOGOUT; cải chính khẳng định CŨ 'thiếu DocPerm → dispatcher-403 status-line' nay SAI theo ADR-IMM00-LIST-SCOPE §9 D8/D9/D10 [read-gate 3 lớp ROLE→EXISTS→ROW]; 0 path/opId/param/schema mới ⇒ oas_baseline GIỮ 105 + components.schemas GIỮ 275, slot {200,401,403} KHÔNG đổi, shape payload success byte-identical; DOC-ONLY mirror quyết định — BE Bước-4 dán khuôn vào services/imm08|imm09|imm11|imm12). 1060→1066. Trừ delta này để giữ pre_fc3_six==191 (transition-baseline post-F-C3 KHÔNG đổi).
        cr86_reschedule_calibration_delta = 9  # AC-CR-86 IMM-11 (2026-07-27, Bước-2 +9 cr86_a..i): `_UPDATE_ALLOWED` @services/imm11.py:1155 KHÔNG chứa `scheduled_date` ⇒ update_calibration NUỐT IM LẶNG (success + 0 thay đổi) ⇒ người dùng buộc hủy+tạo lại → phiếu `Cancelled` rác vào hồ sơ NĐ98 + mất lịch sử; curate `rescheduleCalibration` (+1 path 109→110 · +3 schema 287→290 Request/Response/Envelope · parameters GIỮ 38) + property-add `CalibrationDetail.can_reschedule` (derived-bool, required GIỮ ['name']); đóng mobile CR-81. PENDING-BE: path ở `_PENDING_BE_PATHS` (ngoài _MVP_BUSINESS_PATHS ⇒ 25b parity YAML↔runtime KHÔNG đỏ giả; TRONG 401/403 symmetry) — BE Bước-4 lật cr86_h + chuyển path + refresh cite
        cr85_gate_g04_applicable_delta = 7  # AC-CR-85 IMM-04 (2026-07-27, Bước-2 +7 cr85_a..g): GateStatus += `g04_applicable` (additive, khuôn `g01_waived`) — cổng G04 hết gộp SAI 2 domain (`check_auto_clinical_hold` ghi đè `is_radiation_device=1` cho MỌI phiếu Class C/D ⇒ VR-07 đòi Giấy phép Cục ATBXHN không-thể-tồn-tại: deadlock + ép nộp giấy tờ SAI vào hồ sơ NĐ98; nghĩa vụ Class C/D là NĐ98 Điều 18 do GW-2 gác qua IMM-05). Khoá LUẬT ĐỌC 3 TRẠNG THÁI + INV-G04-1 + 7 khoá cũ bất biến; cr85_f/cr85_g PENDING-BE TỰ-LẬT (cite KHÔNG số dòng để cr76_h khỏi ĐỎ → BE Bước-4 land `gate_g04_applies` PHẢI REFRESH numeric + parity 5 điều; lật ≠ thêm TC ⇒ delta GIỮ 7). paths/schemas/parameters GIỮ 109/287/38 — property-add thuần. Đóng mobile CR-58.
        cr84_repair_evidence_photo_delta = 9  # AC-CR-84 IMM-09 (2026-07-27, Bước-2 +9 cr84_a..i; cr84_i là PENDING-BE ⇒ Bước-4 LẬT thành cr84_j, KHÔNG +TC): +9 TC test_mobile_oas.py TestMobileRepairEvidencePhotoContract (RepairWorkOrderDetail += 3 khoá cổng ẢNH BẰNG CHỨNG NĐ98 evidence_photo_required[integer enum[0,1] KHÔNG boolean]/evidence_photo_missing_idxs[array<integer≥1> 1-based == ĐÚNG tập close_work_order từ chối, INV-CMEVID-1]/evidence_photo_total_required + 2 op ghi closeWorkOrder|confirmInspection khai `IMM09-EVIDENCE-PHOTO-REQUIRED` với context.missing_count/missing_idxs + fields.repair_checklist ĐẾN TRÊN HTTP-200 (KHÔNG 417) + miễn trừ cannot_repair=1 — đóng mobile CR-51 kèm CR-15: cổng Class C/D trước đây CHỈ sống ở client VÀ chết vì suy nhóm nguy cơ từ `risk_class` ánh xạ mất mát; nguồn ĐÚNG = `risk_classification` {High,Critical}, '' ⇒ 0. paths/schemas/parameters GIỮ 109/287/38 — property-add thuần; `required` GIỮ ['name'] KHÔNG nới [bất biến '4 *Detail chỉ PK required']; kèm SỬA CITE-ROT 4 chỗ trong 2 op (services/imm09.py:1105/1134, api/imm09.py:105/165) + CẢI CHÍNH cap closeWorkOrder repair.submit→repair.create @api/imm09.py:149)
        cr83_submit_rca_envelope_delta = 7  # AC-CR-83 IMM-12 (2026-07-27, Bước-2 +6 · Bước-4 +1 = cr83_g parity registry): +6 TC test_mobile_oas.py TestMobileSubmitRcaContract cr83_a..f (curate op submitRca — 3 ràng buộc hồ sơ RCA 5-Why/phân công/hoàn tất trước đây frappe.throw TRẦN ⇒ HTTP-417 KHÔNG envelope; +1 path 108→109 + 4 schema SubmitRcaRequest/RcaFiveWhyStepInput/SubmitRcaResponse/SubmitRcaEnvelope 283→287; parameters GIỮ 38; slot CHỈ {200,401,403}; khoá BẤT ĐỐI XỨNG ĐỌC≠GHI corrective_action ↔ corrective_action_summary)
        cr82_cm_available_actions_delta = 9  # AC-CR-82 IMM-09 (2026-07-27, Bước-2 +8 cr82_a..h & Bước-4 BE +1 cr82_i = 9): +8 TC test_mobile_oas.py TestMobileRepairAvailableActionsParity cr82_a..h (RepairWorkOrderDetail += available_actions[] 6 CTA server-driven — đóng NỬA CM của mobile CR-74, mirror AC-CR-77 nửa PM; TÁI DÙNG AvailableAction ⇒ 0 schema/path/parameter mới)
        cr81_asset_dossier_file_delta = 8  # AC-CR-81 IMM-05 (2026-07-27): +8 TC test_mobile_oas.py TestMobileAssetDossierFileContract cr81_a..h (AssetDossierDocItem 13→18 khoá: bồi 5 khoá TỆP file_url/file_name/file_size/is_private/has_file batch-resolve 1-query từ DocType `File`, luật LINK MỒ CÔI has_file=0 ∧ file_url="" — KHÔNG phát link chết; paths GIỮ 108 · schemas GIỮ 283 · parameters GIỮ 38; cr75_g SUPERSEDE 13→18 khoá).
        cr80_assignable_users_delta = 8  # AC-CR-80 IMM-00 (2026-07-27): +8 TC test_mobile_oas.py TestMobileAssignableUsersContract cr80_a..h (curate op listAssignableUsers — picker theo NĂNG LỰC (capability/DocPerm) + hết cắt IM LẶNG: data OBJECT {items,total,truncated,limit}; đóng mobile CR-75; +1 path 107→108 · +2 schema 281→283 · parameters GIỮ 38)
        cr79_filter_key_whitelist_delta = 8  # AC-CR-79 IMM-08+IMM-09 (2026-07-27): +8 TC test_mobile_oas.py TestMobileWorkOrderFilterKeysContract cr79_a..h (TÁCH `WorkOrderFilters` dùng-chung-3-op → +2 components.parameters `PmWorkOrderFilters`(16 khoá)/`RepairWorkOrderFilters`(18 khoá) khai ĐÚNG tập khoá được honor + câu hành vi 'khoá ngoài danh sách ⇒ 400 IN-ENVELOPE trên HTTP-200, KHÔNG HTTP-500'; `WorkOrderFilters` ở lại phục vụ `listCalibrations` KÈM cảnh báo 'IMM-11 CHƯA whitelist' — ADR-IMM08-FILTERKEY-01; cr79_d/e parity marker OAS ↔ `services.imm08/imm09._ALLOWED_FILTER_KEYS` import THẬT ⇒ 0 bản chép tay; cr79_g cite-parity dùng `_cr79_symbol_spans` quét CẢ Assign module-level; paths GIỮ 107 · schemas GIỮ 281 · parameters 36→38 · 0 opId mới). 1102→1110. Trừ delta này để giữ pre_fc3_six==191 (transition-baseline post-F-C3 KHÔNG đổi).
        cr78_repair_spare_parts_delta = 8  # AC-CR-78 IMM-09 (2026-07-27): +8 TC test_mobile_oas.py TestMobileRepairSparePartsContract cr78_a..h (RepairWorkOrderDetail += spare_parts_used[] $ref RepairSparePartUsedItem + parts_pending_stock_entry; đóng mobile CR-71 "gõ mã mò" + badge XANH GIẢ cho ref treo; stock_entry_ok INTEGER enum[0,1] KHÔNG bool; cr78_g parity enum ↔ services.imm09._STOCK_ENTRY_STATUS import THẬT; cr78_e cite-parity ghim _spare_row_stock_status/validate_spare_parts_stock_entries)
        cr77_pm_available_actions_delta = 9  # AC-CR-77 IMM-08 (2026-07-26): +9 TC test_mobile_oas.py TestMobilePmAvailableActionsParity cr77_a..i (PmWorkOrderDetail += available_actions[] 4 CTA server-driven [start_work, submit_result, reschedule, report_major_failure] — TÁI DÙNG $ref AvailableAction ⇒ 0 schema mới, paths GIỮ 107 / schemas GIỮ 280; optional ∉ required + additionalProperties GIỮ true ⇒ 0 client gãy; `Cancelled` KHÔNG BAO GIỜ là action [đích hợp lệ trong _PM_VALID_TRANSITIONS nhưng 0 endpoint hủy]; cr77_h cite-parity quét SCHEMA (cr74_g chỉ quét description của OP) + cr77_i parity key OAS ↔ services.imm08._PM_ACTION_SPECS). 1085→1094. Trừ delta này để giữ pre_fc3_six==191 (transition-baseline post-F-C3 KHÔNG đổi).
        cr76_gate_status_delta = 8  # CR-76 IMM-04 (2026-07-26): +8 TC test_mobile_oas.py TestMobileGateStatusContract cr76_a..h (curate op getGateStatus — đóng nửa `getGateStatus` của CR-53, mobile hết wire RAW; +1 path 106→107 + 2 schema GateStatus/GateStatusEnvelope 278→280 + 1 param `name` required; 7 khoá boolean THẬT (KHÔNG integer 0|1) tất cả ∈ required; +1 khoá additive g01_waived phân biệt "đủ hồ sơ" vs "thiếu nhưng có giải trình"; ngữ nghĩa BLOCKING-parity: true = cổng KHÔNG chặn (KHÔNG phải "đã hoàn tất"); g02_facility khai rõ là cổng THAM KHẢO 0 enforcement ⇒ cấm gate CTA; 403 IN-ENVELOPE trên HTTP-200 KHÔNG status-line + 0 existence-oracle; cr76_h cite-parity ghim 4 predicate enforcement validate_gate_g01/transition_state/_count_open_ncs/validate_gate_g05_g06 — refactor BR-04-15 dịch dòng ⇒ BE PHẢI refresh cite)
        cr75_asset_dossier_delta = 10  # CR-75 IMM-05 (2026-07-26): +10 TC test_mobile_oas.py TestMobileAssetDossierContract cr75_a..j (curate op getAssetDocuments — op imm05 ĐẦU TIÊN của OAS mobile; +1 path 105→106 + 3 schema AssetDossier/AssetDossierDocItem/AssetDossierEnvelope 275→278 + 1 param `asset` required; documents = GROUPED-OBJECT map doc_category→array<AssetDossierDocItem> KHÔNG items[]/pagination; is_compliant/is_expired/is_exempt integer enum[0,1] KHÔNG boolean CR-01; document_status enum ĐÚNG 5 giá trị SSoT _compute_document_status — thu hồi cặp 'Complete'/'Incomplete'; guard cite-parity BE↔OAS + parity khoá required ⇔ dict-literal return AST; BE CÙNG VÒNG services/imm05.py:548-633). 1067→1077. Trừ delta này để giữ pre_fc3_six==191 (transition-baseline post-F-C3 KHÔNG đổi).
        cr73a_spare_identity_delta = 6  # CR-73(a) IMM-09 (2026-07-25): +6 TC test_mobile_oas.py TestMobileSearchSparePartItemIdentity cr73a_a..f (SearchSparePartItem 10→13 property + required 10→13 — bồi 3 khoá NHẬN DẠNG device_model/device_model_name/spare_part; khử nhập-nhằng phụ-tùng trùng tên giữa 2 IMM Device Model + cấp khoá AC Spare Part cho Gate-2 IMM-09→IMM-15; 0 path/opId/param mới ⇒ oas_baseline GIỮ 105; components.schemas +0). 1054→1060.
        cr69_history_truncation_delta = 6  # CR-69 IMM-08/09/12 (2026-07-25): +6 TC test_mobile_oas.py TestMobileHistoryTruncationContract cr69_a..f (hợp đồng TRUNG THỰC khi cắt cho 3 endpoint device-profile history — AssetPmHistoryEnvelope/AssetRepairHistoryEnvelope/AssetIncidentHistoryEnvelope bồi {total:int, truncated:int enum[0,1]} ADDITIVE vào data.properties; required GIỮ NGUYÊN ⇒ 0 breaking; 0 path/opId/param mới ⇒ oas_baseline GIỮ 105). 1048→1054.
        cr65_repair_checklist_delta = 7  # CR-65 IMM-09 (2026-07-25): +7 TC test_mobile_oas.py TestMobileRepairChecklistItemTyped cr65_a..g (RepairWorkOrderDetail.repair_checklist[] = array $ref RepairChecklistItem MỚI — nối chuỗi typed đọc→đính-ảnh qua khoá idx:integer; 0 path/opId mới ⇒ KHÔNG đụng oas_baseline).
        clinimpact_cr63_delta = 3  # CR-63 IMM-12 (2026-07-24): +3 TC test_mobile_oas.py TestMobileReportIncidentClinicalImpactContract clinimpact_a..c (ReportIncidentRequest bồi property optional clinical_impact string+nullable BR-12-01/NĐ98 Đ67; ∉ required GIỮ EXACT 4; schema OPEN GIỮ; path/opId/whitelist 0 mới ⇒ oas_baseline GIỮ; CONTRACT-ONLY mirror 0 .py-prod — BE clinical_impact đã LIVE @api/imm12.py:95 + services/imm12.py:559). Trừ delta này để giữ pre_fc3_six==191 (transition-baseline post-F-C3 KHÔNG đổi).
        pre_fc3_six = (
            guard_suite_final
            - cr86_reschedule_calibration_delta
            - cr85_gate_g04_applicable_delta
            - cr84_repair_evidence_photo_delta
            - cr83_submit_rca_envelope_delta
            - cr82_cm_available_actions_delta
            - cr81_asset_dossier_file_delta
            - cr80_assignable_users_delta
            - cr79_filter_key_whitelist_delta
            - cr78_repair_spare_parts_delta
            - cr74_detail_readgate_delta
            - cr77_pm_available_actions_delta
            - cr76_gate_status_delta
            - cr75_asset_dossier_delta
            - cr73a_spare_identity_delta
            - cr69_history_truncation_delta
            - cr65_repair_checklist_delta
            - clinimpact_cr63_delta
            - available_actions_incdetail_delta
            - enrich_incdetail_delta
            - needs_write_family_delta
            - warranty_detail_parity_delta
            - needs_read_family_delta
            - alloc_write_delta
            - audit_write_delta
            - label_print_delta
            - attach_photo_idempotency_delta
            - recall_fsca_delta
            - pending_approvals_inbox_delta
            - submit_baseline_action_delta
            - pm_dashboard_stats_delta
            - pm_calendar_delta
