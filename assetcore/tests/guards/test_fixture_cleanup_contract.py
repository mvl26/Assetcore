...
``bench run-tests --module assetcore.tests.imm00.test_imm00`` để lại **18 Asset
...
Run:
    bench --site miyano run-tests --app assetcore \
        --module assetcore.tests.guards.test_fixture_cleanup_contract
"""
...
from assetcore.tests._helpers import _asset_cleanup
from assetcore.tests._helpers._asset_cleanup import decommission_via_closure, purge_asset
...
class TestPurgeAssetRemovesDecommission(FrappeTestCase):
...
class TestNoDuplicatePurgeHelper(FrappeTestCase):
    ...
    def test_asset_heavy_modules_have_module_level_safety_net(self):
        ...
        missing = []
        for mod_name in ("assetcore.tests.imm00.test_imm00", "assetcore.tests.imm05.test_imm05"):
            mod = importlib.import_module(mod_name)
