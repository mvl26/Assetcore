...
Run:
  bench --site miyano run-tests --app assetcore \
      --module assetcore.tests.guards.test_workflow_submit_gate
"""
...
def _source_workflow_files() -> list[str]:
    ...
    # `list_files(..., min_count=)` NÉM LỖI khi thư mục hụt file — `glob` trần trả []
    # âm thầm và mọi khẳng định "không có vi phạm" thành đúng-rỗng-tuếch (§5.2 N5).
    return list_files(WORKFLOW_DIR, ".json", min_count=18, recursive=False)
