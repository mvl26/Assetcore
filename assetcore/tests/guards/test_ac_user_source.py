...
Run:
    bench --site miyano run-tests --app assetcore \
        --module assetcore.tests.guards.test_ac_user_source
"""
...
class _UserFixtureCase(FrappeTestCase):
...
class TestNotificationCoverageScope(FrappeTestCase):
...
class TestLegacyLeakSurfacesRemoved(FrappeTestCase):
...
class TestNoRawUserQueryGuard(FrappeTestCase):
