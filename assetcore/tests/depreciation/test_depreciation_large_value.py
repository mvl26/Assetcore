...
Run:
  bench --site miyano run-tests --app assetcore \
      --module assetcore.tests.depreciation.test_depreciation_large_value
"""
...
class TestMoneyFieldsAreDecimalNotInt(FrappeTestCase):
...
class TestMoneyColumnsWidenedToLongLong(FrappeTestCase):
...
class TestLargeValueSchedulePure(FrappeTestCase):
...
class TestLargeValueAssetRoundTrip(FrappeTestCase):
