...
Run:
    bench --site miyano run-tests --app assetcore \
        --module assetcore.tests.integration.test_password_policy
"""
...
class TestFeedbackTranslation(FrappeTestCase):
...
class TestCreateSystemUserRejectsWeakPassword(FrappeTestCase):
...
class TestOtherPasswordPathsAreVietnamese(FrappeTestCase):
