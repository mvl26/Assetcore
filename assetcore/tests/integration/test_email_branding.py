...
Run:
    bench --site miyano run-tests --app assetcore \
        --module assetcore.tests.integration.test_email_branding
"""
...
class TestRenderEmail(FrappeTestCase):
...
class _MailCaptureBase(FrappeTestCase):
...
class TestDisableFrappeEmailBranding(FrappeTestCase):
