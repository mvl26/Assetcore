...
Run:
  bench --site miyano run-tests --app assetcore \
      --module assetcore.tests.test_audit_form_validation
"""
...
class TestSupplierEmailValidation(unittest.TestCase):
...
class TestAssetNegativePriceGuard(unittest.TestCase):
...
class TestDepreciationStartFallback(unittest.TestCase):
