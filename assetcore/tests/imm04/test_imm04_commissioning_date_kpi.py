...
Run: bench --site miyano run-tests --app assetcore \
        --module assetcore.tests.imm04.test_imm04_commissioning_date_kpi
"""
...
class TestStampHelperUnit(FrappeTestCase):
...
class TestReleasedThisMonthKpi(FrappeTestCase):
...
class TestStampWiredInAllReleasePaths(FrappeTestCase):
...
class TestNoModifiedAnchorInKpi(FrappeTestCase):
