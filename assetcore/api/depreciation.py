...
def _parse_depr_config(asset: dict) -> tuple[float, float, int, object, str] | None:
    """Extract validated depreciation parameters. Returns None when not configured."""
    ...
    if not method or method == "None" or gross <= 0 or years <= 0 or not start:
        return None
    return gross, residual, years, start, method


def _compute_one(asset: dict) -> dict:
    cfg = _parse_depr_config(asset)
    if cfg is None:
        gross = float(asset.get("gross_purchase_amount") or 0)
    ...
    gross, residual, years, start, method = cfg
...
def _accumulate_stats(acc: dict, asset: dict) -> None:
    gross    = float(asset.get("gross_purchase_amount") or 0)
    computed = _compute_one(asset)
    acc["total_gross"] += gross
    if not computed["configured"]:
        acc["unconfigured"] += 1
        acc["total_book"]   += gross
        return
    bv = computed["book_value"]
    acc["configured"]        += 1
    acc["total_accumulated"] += computed["accumulated"]
    acc["total_book"]        += bv
    if bv <= float(asset.get("residual_value") or 0) + 1:
        acc["fully_depreciated"] += 1
    m   = (asset.get("depreciation_method") or "Khác").strip()
    cat = asset.get("asset_category") or "Chưa phân loại"
    acc["by_method"][m]   = acc["by_method"].get(m, 0) + 1
    acc["by_category"][cat] = acc["by_category"].get(cat, 0.0) + bv
...
def get_depreciation_stats() -> dict:
    ...
    offset = 0
    while True:
        batch = frappe.get_all(_DT_ASSET, fields=_DEPR_FIELDS,
                               limit_start=offset, limit_page_length=_BATCH)
        if not batch:
            break
        asset_count += len(batch)
        for a in batch:
            _accumulate_stats(totals, a)
        if len(batch) < _BATCH:
            break
        offset += _BATCH
    ...
    return _ok({
        ...
        "by_category":        sorted(
            [{"category": k, "book_value": v} for k, v in totals["by_category"].items()],
