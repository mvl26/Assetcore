...
IMM-04 chuẩn hóa pipeline 11 bước từ tiếp nhận thiết bị đến phát hành lâm sàng (Clinical Release), thay thế quy trình giấy/Excel rời rạc bằng workflow có gate kiểm soát (G01–G06), validation rule (VR-01 → VR-07) và audit trail bất biến. Quy trình bao trùm 3 nhóm bên (NCC/Vendor — KTV/Biomed — QA/BGĐ) và đảm bảo: (1) không thiết bị nào vận hành lâm sàng nếu chưa qua đủ gate, (2) mọi action sinh lifecycle event, (3) Asset record chỉ được mint khi Submit thành công.

Sub-section dưới đây trình bày As-Is (II.2), pain points (II.3), To-Be với swimlane (II.4), decision points (II.5), metrics (II.6), RACI (II.7), exception flow (II.8), so sánh (II.9), và activity diagram cho UC chính (II.10).
