...
> Nếu update template map → phải chạy lại `docs/res/imports/generate_templates.py`
...
- **LL-IMP-2** (SSoT display-link 1 nguồn 3 hướng, 2026-08-11): map display→code sống ở `utils/import_helpers.LINK_DISPLAY_BY_DOCTYPE`; `api/import_data._RESOLVABLE_LINKS_BY_DOCTYPE` chỉ là **alias** (test `assertIs`). Map này phục vụ **3** hướng, sửa 1 nơi phải ăn cả 3: (1) resolver tên→mã trước insert; (2) `_link_lookup_set` accept cả tên lẫn mã; (3) **export in TÊN** (`resolve_links_to_display`). Bỏ sót (3) = file "Xuất Excel" đầy mã hệ thống, người dùng nhập lại phải đi tra mã — đúng lỗi user báo 2026-08-11. Guard: `tests/test_import_display_names.py::test_every_exported_link_column_is_in_ssot` quét meta Link field của MỌI doctype hỗ trợ.
- **LL-IMP-3** (số hàng thật, không phải chỉ số dòng, 2026-08-11): parser LOẠI dòng trống ⇒ "dòng thứ N" của validator KHÔNG bằng hàng Excel `N+5`. `_rows_to_dicts` ghi `SOURCE_ROW_KEY="__source_row__"` = số hàng gốc; `enrich_issues(doctype, rows, issues)` bồi `source_row` + `label` (nhãn VI) vào MỌI lỗi trả ra FE (preview · import errors · skipped_rows · error report xlsx). Mọi consumer ghi DB PHẢI bỏ khoá `__*` (`_normalise_row`) — lọt vào `doc.update()` là nổ lúc insert.
- **LL-IMP-4** (skip là quyền của người dùng, không phải đặc quyền của doctype, 2026-08-11): 'Bỏ qua dòng lỗi/trùng' phải chạy cho MỌI doctype hỗ trợ. Chặn theo doctype (cũ: `if doctype == "User": raise`) = người dùng có 1 email sai phải sửa file rồi nhập lại từ đầu. Đường upsert riêng vẫn phải nhận `invalid_idx` + `skipped_rows` và `continue` đúng dòng.
- **LL-IMP-5** (banner template phải trỏ đúng hàng dữ liệu đầu tiên, 2026-08-11): parser đọc từ hàng `FIRST_DATA_ROW = 6`; banner từng ghi "từ hàng 5" ⇒ người dùng ghi đè hàng ví dụ và dòng đầu bị nuốt IM LẶNG (không lỗi, không cảnh báo). Số hàng trong banner + vùng dropdown + `FIRST_DATA_ROW` phải cùng một con số; guard `test_banner_points_at_the_first_real_data_row`.

- **LL-IMP-6** (DocType có BẢNG CON nhập theo NHÓM, 2026-08-11): dữ liệu cha+con (vd `PM Checklist Template` + `PM Checklist Item`) dùng **file PHẲNG — mỗi hàng = 1 dòng bảng con**, cột của cha lặp lại ở mọi hàng. Khai ở `_REF_DATA_CONFIG` các khoá `child_table` · `group_key_fields` · `parent_fields` · `child_fields` ⇒ tự vào `GROUPED_IMPORT_DOCTYPES` và `_do_import` rẽ sang `_do_import_grouped`. Bốn luật bắt buộc: **(a)** khoá nhóm phải CHUẨN HOÁ trước khi gộp (tên→mã qua `_resolve_links`, nhãn VI→giá trị lưu qua `enum_to_stored`) — nếu không, cùng một mẫu viết bằng tên ở dòng này và bằng mã ở dòng kia thành 2 nhóm rồi đụng khoá; **(b)** gộp theo KHOÁ, không theo vị trí liền kề — người dùng hay để các hàng cùng mẫu rải rác; **(c)** cột cha lấy theo hàng ĐẦU nhóm, khai lệch ở hàng sau chỉ **warning** (chặn = file thật không bao giờ nhập nổi); **(d)** tạo bản ghi qua **service layer** của module (`imm08.create_template`), KHÔNG `new_doc().insert()` trần — quy tắc nghiệp vụ + đánh số hạng mục con nằm ở đó. Trả thêm `groups_created` vì `success` đếm theo DÒNG, không phải theo bản ghi. Export phải trải phẳng CÙNG bố cục (`_export_pm_checklist_templates`) mới round-trip được.
- **LL-IMP-7** (cột Select cũng phải hỏi bằng tiếng Việt — **có guard 4 tầng**, 2026-08-11): SSoT `ENUM_DISPLAY_BY_DOCTYPE` (`utils/import_helpers.py`) + 3 hàm `enum_display` / `enum_accepted` / `enum_to_stored`. Template + export in **nhãn VI** ("Hàng quý", "Số đo"); import nhận **cả** nhãn VI lẫn giá trị gốc rồi đổi về giá trị gốc trước khi lưu — **KHÔNG đụng enum của DocType** (LL-FE-53). Nhãn phải khớp SSoT FE (`formatters.ts::PM_TYPE_MAP`, `labels.ts::MEASUREMENT_TYPE_LABELS`); lệch = người dùng đọc màn hình một kiểu, điền file một kiểu. Câu báo lỗi liệt kê lựa chọn phải liệt kê **nhãn VI**, không liệt kê enum tiếng Anh.
  **Guard `assetcore/tests/guards/test_import_enum_labels.py` (7 TC / 4 tầng)** — thêm 1 cột Select mới mà quên nhãn là ĐỎ ngay, không cần nhớ luật:
  1. *Phủ kín* — mọi cột Select xuất hiện trong file mẫu **hoặc** `export_fields` phải có nhãn cho ĐỦ `options` của DocType.
  2. *Không khoá rác* — mọi khoá nhãn phải là `option` CÓ THẬT (bắt lỗi gõ sai enum).
  3. *Parity FE* — nhãn phải trùng SSoT hiển thị (`constants/labels.ts` · `utils/formatters.ts`), đọc THẲNG map trong `.ts`.
  4. *File thật* — dropdown + ô ví dụ trong `.xlsx` đã sinh phải là nhãn VI.

  **Ba bẫy guard này đã bắt được ngay hôm khai sinh — đừng lặp lại:**
  - **Hai field khác tập lựa chọn dùng chung 1 map**: `AC Supplier.supplier_group` kết thúc `Service Provider`, `vendor_type` kết thúc `Service`. Gộp = sinh khoá rác + 2 nhãn TRÙNG trong một cột ⇒ `enum_to_stored` đổi ngược không xác định. Luôn đọc `options` của TỪNG field.
  - **Nhãn có nhưng dropdown thiếu lựa chọn**: `AC Asset.depreciation_method` có option `None` (= không khấu hao); khai nhãn mà quên thêm vào dropdown ⇒ người dùng không chọn được. Dropdown phải sinh TỪ map, đừng gõ tay lần hai.
  - **Dropdown chào giá trị KHÔNG tồn tại**: sheet SLA từng chào `P1 Critical`/`P1 High` trong khi `IMM SLA Policy.priority` chỉ có `P1..P4` ⇒ người dùng làm đúng hướng dẫn mà hệ thống vẫn từ chối. Danh sách trong file mẫu phải đối chiếu `options` thật của DocType.
- **LL-IMP-8** (Việt hoá enum phải làm CẢ LOẠT, không làm từng mẫu, 2026-08-11): làm nửa vời (1 mẫu VI, 7 mẫu EN) **khó dùng hơn thuần tiếng Anh** — người dùng không đoán được cột nào theo luật nào, và mỗi lần thêm mẫu lại quên. Khi Việt hoá một cột Select: quét `_REF_DATA_CONFIG` + `_TEMPLATE_MAP` một lượt, khai hết vào `ENUM_DISPLAY_BY_DOCTYPE`, đổi validator sang `BaseImportValidator._check_enum` (bỏ mọi `_VALID_*` hardcode tiếng Anh), thêm `_restore_enum_values` vào đường insert phẳng, rồi sinh lại TOÀN BỘ template. Bằng chứng phải là guard xanh, không phải "đã sửa mẫu vừa đụng".

- **LL-IMP-9** (file XUẤT RA phải là file NHẬP LẠI ĐƯỢC — khung file là hợp đồng, 2026-08-11): `export_ref_data` từng ghi khung **2 hàng** (nhãn + fieldname, dữ liệu từ hàng 3) trong khi parser luôn bỏ **5** hàng ⇒ vòng "Xuất → sửa → Nhập lại" **nuốt IM LẶNG 3 bản ghi đầu** (đo thật: xuất 78 danh mục, nhập lại đọc 75; không lỗi, không cảnh báo). Ba luật:
  **(a)** Export ghi **ĐÚNG khung 5 hàng của file mẫu** (banner · fieldname · nhãn VI · mô tả · ví dụ) — một bố cục duy nhất cho cả mẫu lẫn file xuất; hàng 2 là fieldname ở cả hai.
  **(b)** Banner mang tiền tố `TEMPLATE_BANNER_PREFIX = "📋 HƯỚNG DẪN IMPORT"` và **`detect_first_data_row`** dựa vào nó: có banner ⇒ hàng 6; không banner nhưng hàng 3 khớp hàng nhãn ⇒ hàng 6; còn lại ⇒ `LEGACY_EXPORT_FIRST_DATA_ROW = 3`. **Không được bỏ nhánh legacy**: file xuất bố cục cũ đã nằm trong máy người dùng.
  **(c)** `_rows_to_dicts(fieldnames, rows, first_data_row)` nhận số hàng đầu **qua tham số**, không đọc hằng — lệch 1 hàng là mọi câu báo lỗi chỉ nhầm chỗ.
  Guard: `assetcore/tests/import_data/test_import_file_layout.py` (5 TC). **Bất biến phải kiểm theo SỐ LƯỢNG bản ghi** (`frappe.db.count` vs số dòng parser đọc), KHÔNG theo tên vài bản ghi — bản ghi `_TEST` thường nằm CUỐI file nên phép so theo tên xanh giả trong khi 3 hàng đầu đã mất.

- **LL-IMP-10** (nhập theo NHÓM: dạy cách điền + xem trước theo bản ghi + cập nhật là opt-in, 2026-08-11): mở rộng LL-IMP-6 sau câu hỏi thật của người dùng *"điền nhiều checklist thì điền sao được?"*. Bốn luật:
  **(a) File mẫu phải DẠY được cách điền nhiều bản ghi.** Một hàng ví dụ đơn lẻ không cho thấy "cột cha lặp lại" lẫn "bản ghi thứ hai nằm ngay dưới". Thêm sheet **"Ví dụ minh hoạ"** (≥2 nhóm × ≥3 dòng con, 2 khối màu, dòng "KẾT QUẢ: N hàng ⇒ M bản ghi"). **KHÔNG chèn khối ví dụ nhiều hàng vào sheet nhập liệu** — sẽ phải đổi hàng bắt đầu dữ liệu theo sheet, đúng bẫy LL-IMP-5. Sheet ví dụ bắt buộc đi kèm entry trong `_SHEET_NAME_MAP` để parser bám sheet dữ liệu (người dùng lưu file lúc đang đứng ở sheet ví dụ là chuyện thường); `wb.active = 0`.
  **(b) Bước xem trước phải tóm tắt theo NHÓM**, không chỉ theo dòng: `_summarise_groups` trả `groups[]` = tên bản ghi · khoá đã dịch sang chữ người đọc (`_group_key_display`: mã→TÊN, enum→nhãn VI) · số dòng con **đã trừ hàng lỗi** · `exists` · `existing_items` · `action` (`create|update|blocked`) · `first_source_row`. Nhóm có khoá rỗng phải có tên gọi ("(thiếu thông tin nhận dạng)"), đừng để bảng hiện dòng trống.
  **(c) Cập nhật bản ghi đã có = opt-in, và server là nơi quyết định.** `update_existing` đi qua **cả** `preview_ref_data` lẫn `import_ref_data`; validator đổi lỗi trùng → **cảnh báo** kèm số dòng con sẽ mất ("THAY toàn bộ N hạng mục bằng M"), + cảnh báo riêng nếu bản ghi đã `approved_by` (hồ sơ QMS). FE bật/tắt là **gọi lại preview**, KHÔNG tự lọc lỗi phía FE (hai nguồn sự thật = lệch lúc nào không biết). Ghi qua service layer `update_template`, KHÔNG `db_set` bảng con. Kết quả trả thêm `groups_updated`.
  **(d) Cờ boolean từ HTTP phải ép kiểu bằng `_as_bool`** — `cint("true") == 0`, dùng `cint` là bật/tắt ngược. Cờ ghi đè luôn gửi tường minh từ FE, không để `undefined` trôi xuống BE.
  Phụ: `_group_key_of(..., cache)` nhận cache tra Link cho cả file (file 300 hàng nếu không có cache bắn ~600 truy vấn cho vài giá trị lặp); dropdown file mẫu phủ tới `LAST_DV_ROW = 1000` (10 mẫu × 15 hạng mục đã vượt 105 hàng cũ).

- **LL-IMP-11** (nhập để CẬP NHẬT bản ghi đã có — luật chung cho mọi loại dữ liệu, 2026-08-11): "Xuất → sửa trong Excel → Nhập lại" là cách bệnh viện sửa hàng loạt; chặn cứng "đã tồn tại" = bắt mở từng bản ghi trên màn hình. Sáu luật:
  **(a) SSoT `UPDATE_KEY_BY_DOCTYPE`** (`utils/import_helpers.py`): cột nhận dạng theo THỨ TỰ ƯU TIÊN, **mã trước tên sau** — khoá theo tên thì đổi tên bằng import = đẻ bản ghi mới, mất dấu bản cũ. Khoá phải TRÙNG cột validator dùng để báo trùng; lệch = báo "đã tồn tại" một đằng, ghi đè một nẻo.
  **(b) `find_existing_by_key` gom truy vấn theo LÔ** (một câu `IN (...)` cho mỗi cột khoá). Tra từng dòng = file 300 hàng bắn 300 truy vấn.
  **(c) Một chỗ quyết định 'trùng nghĩa là gì'**: `BaseImportValidator._dup_existing` — tắt công tắc ⇒ lỗi chặn kèm câu "bật 'Cập nhật bản ghi đã có'"; bật ⇒ cảnh báo. Mọi validator gọi helper này, KHÔNG tự `self._err(... "đã tồn tại" ...)`.
  **(d) Ô ĐỂ TRỐNG = GIỮ NGUYÊN giá trị cũ** (`_update_flat_record` lọc `value not in ("", None)`). Người dùng xuất 8 cột sửa 1 cột; coi trống là "xoá" là mất 7 cột. ⚠️ Khác hẳn bảng con của DocType nhóm (LL-IMP-10c): ở đó file là bản chuẩn, dòng con cũ bị THAY SẠCH — đừng đem luật nọ áp luật kia.
  **(e) Cột nhạy cảm khoá bằng `UPDATE_LOCKED_FIELDS_BY_DOCTYPE`** (AC Asset: `asset_code`·`manufacturer_sn`·`lifecycle_status`·`status`·`qr_token`) — trạng thái vòng đời đi kèm workflow + audit trail NĐ98. Bỏ qua phải **CẢNH BÁO Ở BƯỚC XEM TRƯỚC** (`_locked_field_warnings`), không im lặng: người dùng sửa trạng thái trong file, thấy "nhập thành công", tưởng đã đổi.
  **(f) Mọi doctype nhập được PHẢI có validator.** `AC Supplier` từng không có ⇒ `naming_series` + `supplier_code` là cột tuỳ chọn ⇒ nhập lại cùng file đẻ bản ghi trùng, im lặng (đo: `AC-SUP-2026-1541`/`1542`). Guard `test_validator_exists_for_every_importable_doctype`.
  Preview trả `will_create` / `will_update` / `existing_rows` (đếm cả khi công tắc TẮT — nhìn `will_update` lúc tắt luôn ra 0 thì giao diện không bao giờ mời chào bật). Import trả `updated`.
  ⚠️ **Bẫy test đã dính**: (1) CSV nội dung y hệt lần chạy trước ⇒ Frappe dedupe `File` theo `content_hash`, trả về đường dẫn đã bị dọn ⇒ `save_file` nổ `FileNotFoundError` — nhét chuỗi ngẫu nhiên vào hàng banner; (2) dọn `AC Asset` phải dùng `tests/_asset_cleanup.purge_asset` (WR-03 chặn xoá cứng, `force=True` KHÔNG bỏ qua `on_trash` tuỳ biến) — dọn hụt là lần chạy sau đỏ vì trùng mã, không phải vì code sai; (3) mã tài sản trong fixture KHÔNG được bắt đầu bằng `_` (`_RESERVED_NAME_PREFIX`).
