...
**Test coupling (RED thực 2026-07-01):** spell-out 1 nhãn **status/SSoT** (`RCA Required`→"Cần phân tích nguyên nhân gốc", `SLA`→"cam kết dịch vụ") vỡ **≥5 file test tên KHÁC, module KHÁC** (`constants/incidentLabels.test.ts` · `utils/formatters.test.ts` · `views/incident/incidentListDrilldown.test.ts` · `views/incident/slaBreachLiveSoT.test.ts` · `views/cm/cmSlaClockStop.test.ts` · `cmSlaBreachedDivergence.test.ts`) — colocated-check MISS hết. Fix: **grep literal CŨ toàn repo (kể cả `*.test.ts`)** rồi cập nhật mọi assertion cùng pass; full `vitest run` (KHÔNG `--changed`/colocated).
