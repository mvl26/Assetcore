...
| Lỗi "live" sau khi vừa sửa code (vd "Không thể tạo PDF nhãn") + định phóng factory fix | Nghi STALE-WORKER trước khi nghi bug code (2026-06-11) | `curl -X POST http://localhost:8000/api/method/<dotted.path> -H "Host: <site>" -d '{...}'` → nếu **417 "no attribute '<fn>'"** = stale → **USER `bench restart`** (+ `clear-cache`), **KHÔNG factory/viết-fix** (LL-DEPLOY-07). So `ps … lstart` (boot) vs `stat -c %y *.py` (mtime). |
