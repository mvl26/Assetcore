...
describe('INV-UX4L1-2 — mã TC nối tiếp 24…31 và bảng test §12.6 khớp sổ', () => {
  ...
  it('§12.6 khai đủ 8 file test, khớp mã TC của sổ, đặt tên <Nguồn>.states.test.ts', () => {
    ...
    expect(bad, 'file test trạng thái phải đặt tên <Nguồn>.states.test.ts (SPEC §5.1)').toEqual([])
  })
})
