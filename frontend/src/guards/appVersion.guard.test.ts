...
import { FRONTEND_ROOT, GUARDS, REPO_ROOT, SRC, listFiles } from '@/test/paths'
import { describe, it, expect } from 'vitest'
import { APP_VERSION, APP_VERSION_LABEL, APP_VERSION_FULL_LABEL } from '@/constants/appVersion'
...
// SSoT phiên bản + CHÍNH guard này (guard phải chứa chuỗi mẫu để dò được nó).
const SSOT_FILES = new Set([
  resolve(SRC_ROOT, 'constants/appVersion.ts'),
  resolve(SRC_ROOT, 'constants/appVersion.test.ts'),
  resolve(GUARDS, 'appVersion.guard.test.ts'),
])
