...
import { COMPONENTS, REPO_ROOT } from '@/test/paths'
...
describe('TC-BE-OPHACL-04 (nửa FE) — parity hợp đồng cap: ĐÚNG 3 khoá, 0 cap lạ, 0 cap thiếu', () => {
  const text = readFileSync(resolve(COMPONENTS, 'asset', 'AssetOperationalHistory.vue'), 'utf8')
  ...
  it('3 cap của FE == OP_HISTORY_BRANCH_GATE trên đĩa của backend (BE↔FE 0 drift)', () => {
    const beFile = resolve(REPO_ROOT, 'assetcore/services/shared/connection_meta.py')
