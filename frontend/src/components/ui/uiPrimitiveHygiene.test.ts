...
const testFiles = readdirSync(UI_DIR).filter((f) => f.endsWith('.test.ts')).sort()
...
describe('Vệ sinh primitive tầng 0 (TC-UX2-17..19)', () => {
  ...
  it('TC-UX2-18 / INV-UI-2: đúng 8 primitive, mỗi file có đúng 1 test cùng tên, barrel khớp', () => {
    // 3 vế phải khớp khít: số export == số file .vue == số file test (trừ chính guard).
    ...
    for (const name of EXPECTED_PRIMITIVES) {
      const matches = testFiles.filter((t) => t === `${name}.test.ts`)
      expect(matches, `primitive ${name} phải có đúng 1 file test co-located`).toHaveLength(1)
    }
    // ngoài 7 test primitive chỉ được có chính guard này
    expect(testFiles).toHaveLength(EXPECTED_PRIMITIVES.length + 1)
    expect(testFiles).toContain('uiPrimitiveHygiene.test.ts')
