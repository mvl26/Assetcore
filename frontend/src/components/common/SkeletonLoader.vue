...
// AC-UX-036 — adoption LIVE của primitive tầng 0: mọi khối shimmer render QUA
// ui/Skeleton.vue thay vì <div class="skeleton"> rời rạc.
// BẤT BIẾN: API công khai (variant, rows), cấu trúc wrapper, thứ tự/số lượng khối và
// TOÀN BỘ chuỗi class kích thước giữ nguyên ⇒ 49 view tiêu thụ không đổi một dòng
// (guard đếm khối: SkeletonLoader.test.ts).
...
<template>
  ...
  <div v-if="variant === 'kpi-cards'" class="grid grid-cols-2 lg:grid-cols-4 gap-5">
    <div v-for="i in 4" :key="i" class="card p-5 space-y-3">
      ...
      <Skeleton class="h-2 w-full rounded" />
    </div>
  </div>
...
v-for="row in rows" :key="row"
  ...
  <div v-else-if="variant === 'form'" class="space-y-6" aria-busy="true">
    ...
    <Skeleton class="h-px w-full rounded" />
    <div class="space-y-2">
  ...
  <div v-else-if="variant === 'card'" class="card space-y-4" aria-busy="true">
    ...
    <div class="space-y-2.5">
      <Skeleton v-for="i in rows" :key="i" class="h-3.5 rounded" :style="`width: ${70 + (i % 3) * 10}%`" />
    </div>
  </div>
...
v-for="i in rows" :key="i"
      ...
      <div class="flex-1 space-y-1.5">
        <Skeleton class="h-3.5 rounded" :style="`width: ${50 + (i % 4) * 12}%`" />
