...
const navGroups: NavGroup[] = [
  ...
  { key: 'planning', title: 'Kế hoạch Mua sắm (IMM-01/02/03)', icon: 'clipboard', items: [
    ...
    { label: 'Yêu cầu mua sắm',      path: '/planning/purchase-order-requests', icon: 'clipboard' },
  ]},
  { key: 'procurement', title: 'Mua sắm & Tiếp nhận', icon: 'cart', items: [
    ...
    { label: 'Hồ sơ thiết bị',     path: '/documents',     icon: 'folder'    },
  ]},
  ...
  { key: 'eol', title: 'Cuối vòng đời (IMM-13/14)', icon: 'alert', items: [
    { label: 'Ngừng sử dụng', path: '/decommission/list', icon: 'alert'  },
    { label: 'Lưu trữ hồ sơ', path: '/archive/list',      icon: 'folder' },
  ]},
  ...
  >
    ...
    <Transition name="fade-x">
      <div v-if="!collapsed" class="sidebar-footer px-4 py-3">
        <p class="text-[11px] text-slate-500 font-medium">AssetCore v2.0 · Wave 2</p>
      </div>
    </Transition>
  </aside>
</template>
