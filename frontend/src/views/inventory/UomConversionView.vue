...
<template>
  <div class="page-container animate-fade-in">
    ...
    <!-- Toast -->
    <div
v-if="toast" class="mb-4 px-4 py-2.5 rounded-lg text-sm"
          ...
          <div class="flex-1">
            ...
            <details class="mt-3">
              ...
              <div class="mt-2 max-h-48 overflow-y-auto text-xs space-y-0.5">
                <div
v-for="p in partsMissing" :key="p.name"
      ...
      <div class="flex items-center justify-between mb-3">
        <div class="flex gap-2">
          <input
v-model="partsSearch" type="text" placeholder="Tìm phụ tùng..."
          ...
          <div class="grid grid-cols-1 md:grid-cols-5 gap-3 items-end">
            <div>
              ...
              <select v-model="newConv.uom" class="form-input text-sm w-full">
                <option value="">— chọn —</option>
                <option
v-for="u in activeUoms.filter(x => x.name !== convInfo?.stock_uom)"
                        :key="u.name" :value="u.name">
{{ u.uom_name }}
</option>
              </select>
            </div>
            <div>
              ...
              <input
v-model.number="newConv.conversion_factor" type="number" min="0.000001" step="any"
...
{{ convInfo.stock_uom }}
    ...
    <!-- UOM Form Modal -->
    <div
v-if="showUomForm" class="fixed inset-0 bg-slate-900/50 z-50 flex items-center justify-center p-4"
