...
async function submit() {
  ...
  try {
    ...
    if (r?.name) router.push(`/calibration/${r.name}`)
...
<template>
  <div class="page-container animate-fade-in max-w-2xl space-y-5">
    <div class="flex items-center gap-3">
      ...
      <div>
        <p class="text-xs text-slate-400 uppercase tracking-widest">IMM-11</p>
    ...
    <form @submit.prevent="submit" class="card p-5 space-y-4">
      <div class="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input v-model="form.scheduled_date" type="date" class="form-input w-full" required />
        </div>
        <div>
        ...
        <div class="flex items-center gap-2">
          <input v-model="form.is_recalibration" type="checkbox" :true-value="1" :false-value="0" class="h-4 w-4 text-blue-600 rounded" id="recal" />
