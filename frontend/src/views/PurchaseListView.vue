  ...
  <div class="page-container animate-fade-in">
    ...
    <div class="flex items-start justify-between mb-5">
      <div>
        ...
        <h1 class="text-2xl font-bold text-slate-900">Đơn mua hàng</h1>
        <p class="text-sm text-slate-500 mt-1">Tổng <strong class="text-slate-700">{{ total }}</strong> đơn hàng</p>
      </div>
      ...
      <div v-show="showFilters" class="card p-4 mb-4 space-y-3">
        <div class="flex flex-wrap gap-3 items-end">
          <div class="flex items-center gap-2">
            <label for="pur-status" class="text-sm text-slate-600 shrink-0">Trạng thái:</label>
            <select id="pur-status" v-model="statusFilter" class="form-select text-sm" @change="() => { page = 1; load() }">
              <option value="">Tất cả</option>
              <option v-for="s in STATUSES" :key="s.value" :value="s.value">{{ s.label }}</option>
            </select>
          </div>
          <div class="flex items-center gap-2">
            <p class="text-sm text-slate-600 shrink-0">Nhà cung cấp:</p>
            <div class="w-56">
              <SmartSelect v-model="supplierFilter" doctype="AC Supplier" placeholder="Tất cả nhà cung cấp..."
    ...
    <div class="card overflow-hidden">
      ...
      <div v-if="loading && !rows.length" class="text-center py-20 text-slate-400">Đang tải...</div>
        ...
        <tbody>
          ...
          >
            ...
            <td class="px-4 py-3 text-center">
              ...
              >{{ STATUSES.find(s => s.value === r.status)?.label || r.status }}</button>
            </td>
          </tr>
        </tbody>
      </table>
