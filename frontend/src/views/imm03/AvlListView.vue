...
async function doCreate() {
  ...
  await store.api.createAvlEntry(newAvl.supplier, newAvl.device_category, newAvl.validity_years, newAvl.valid_from)
...
<template>
  <div class="page-container animate-fade-in">
    <div class="flex items-start justify-between mb-4">
      <div>
        <h1 class="text-2xl font-bold text-slate-900">Danh mục nhà cung cấp được duyệt</h1>
        <p class="text-sm text-slate-500 mt-1">
          Tổng <strong class="text-slate-700">{{ store.avlEntries.length }}</strong> giấy phép — cấp phép theo nhóm thiết bị, có hạn hiệu lực.
        </p>
      </div>
    ...
    <div class="card overflow-hidden">
      ...
      <div v-if="filteredAvl.length" class="overflow-x-auto">
          ...
          <tbody>
            <tr v-for="a in filteredAvl" :key="a.name">
              ...
              <td class="actions-col">
                <button v-if="a.workflow_state === 'Draft'" class="btn-mini btn-success" @click="approveAvl(a)">Phê duyệt</button>
                <button v-if="a.workflow_state === 'Approved' || a.workflow_state === 'Conditional'"
                        class="btn-mini btn-danger" @click="suspendAvl(a)">Đình chỉ</button>
              </td>
            </tr>
          </tbody>
        </table>
