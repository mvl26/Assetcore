        ...
        <tbody class="divide-y divide-gray-100">
          <tr v-for="f in items" :key="f.name" class="hover:bg-gray-50">
            ...
            <td class="px-4 py-3 text-right space-x-2 whitespace-nowrap">
              <button class="text-blue-600 text-xs font-medium" @click="router.push(`/cm/firmware/${f.name}`)">Chi tiết</button>
              <button class="text-gray-500 text-xs font-medium" @click="openEdit(f.name)">Sửa</button>
              <button class="text-red-600 text-xs font-medium" @click="remove(f.name)">Xóa</button>
            </td>
          </tr>
        </tbody>
      </table>
