...
// Pre-fill asset_ref từ query param (?asset=...) nếu được navigate từ
...
function onCategoryChange() {
  suggestions.value = DOC_TYPE_SUGGESTIONS[form.value.doc_category] ?? []
  form.value.doc_type_detail = ''
}

function selectSuggestion(s: string) {
  form.value.doc_type_detail = s
}

/** Returns a validation error message or null if valid. */
function validateForm(): string | null {
  // asset_ref is optional when is_model_level = 1 (Task 3b)
  const assetRequired = form.value.is_model_level !== 1
  if ((assetRequired && !form.value.asset_ref) || !form.value.doc_type_detail || !form.value.doc_number || !form.value.issued_date) {
    return 'Vui lòng điền đầy đủ các trường bắt buộc (*).'
  }
  if (form.value.is_exempt === 1 && !form.value.exempt_reason.trim()) {
    return 'Vui lòng nhập Lý do miễn khi đánh dấu Miễn đăng ký NĐ98.'
  }
  if (form.value.expiry_date && form.value.issued_date && form.value.expiry_date <= form.value.issued_date) {
    return 'VR-01: Ngày hết hạn phải sau ngày phát hành.'
  }
  if (form.value.version && form.value.version !== '1.0' && !form.value.change_summary?.trim()) {
    return 'VR-09: Cần nhập tóm tắt thay đổi khi phiên bản > 1.0.'
  }
  if (['Legal', 'Certification'].includes(form.value.doc_category) && !form.value.expiry_date) {
    return 'Tài liệu Pháp lý / Kiểm định bắt buộc có Ngày hết hạn.'
  }
  if (form.value.doc_category === 'Legal' && !form.value.issuing_authority) {
...
function buildPayload(fileUrl: string): Record<string, unknown> {
  const payload: Record<string, unknown> = { ...form.value, file_attachment: fileUrl }
...
{{ s }}
        ...
        <div class="form-field">
          ...
          <DateInput id="field-issued-date" v-model="form.issued_date" class="input" />
        </div>
        ...
        <div class="form-field">
          ...
          <DateInput id="field-expiry-date" v-model="form.expiry_date" class="input" />
        </div>
