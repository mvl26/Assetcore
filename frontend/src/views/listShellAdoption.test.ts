...
const SPEC_PATH = resolve(SRC, '../../docs/ui-ux/02_LIST_PAGE_SHELL.md')
...
describe('INV-UX3A-4 — adoption phải kèm test trạng thái', () => {
  it('mỗi view đã áp khuôn có <ten>ListStates.test.ts đặt cạnh nó', () => {
    const missing: string[] = []
    for (const p of adopters) {
      const dir = dirname(resolve(SRC, p))
      // `AssetListView.vue` → `assetListStates.test.ts` · `RCAListView.vue` → `rcaListStates.test.ts`.
      // So khớp KHÔNG phân biệt hoa/thường: quy ước camelCase của repo không đều ở tiền tố viết tắt
      // (RCA, PM, CM, AVL) — ép chữ hoa chính xác sẽ báo nhầm 4 file đang có thật.
      const want = `${basename(p, '.vue').replace(/ListView$/, '')}ListStates.test.ts`.toLowerCase()
