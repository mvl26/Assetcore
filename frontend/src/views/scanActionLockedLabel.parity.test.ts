...
import IncidentCreateView from './incident/IncidentCreateView.vue'
import PMWorkOrderCreateView from './pm/PMWorkOrderCreateView.vue'
import CMCreateView from './cm/CMCreateView.vue'
...
describe('TC-PARITY-NO-CLIENTGETVALUE-4VIEW — loader asset-meta 4 view không còn frappe.client.get_value', () => {
  ...
  it('Incident: KHÔNG nạp asset-meta qua raw RPC (whole-file sạch frappe.client.get_value)', () => {
    const src = fs.readFileSync(path.resolve(__dirname, 'incident/IncidentCreateView.vue'), 'utf8')
