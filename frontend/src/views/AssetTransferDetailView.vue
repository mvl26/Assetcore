    ...
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-3">
        ...
        <template v-if="isPending">
          <button @click="approve" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors">
            Phê duyệt
          </button>
          <button @click="showRejectModal = true" class="bg-red-50 hover:bg-red-100 text-red-600 px-4 py-2 rounded-lg text-sm font-medium transition-colors">
            Từ chối
          </button>
          <button @click="cancel" class="text-gray-400 hover:text-gray-600 text-sm">Hủy phiếu</button>
        </template>
        <template v-if="isApproved">
          <button @click="confirmReceipt" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors">
            Xác nhận tiếp nhận
          </button>
        </template>
      </div>
      ...
      <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
        <div class="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input v-model="form.transfer_date" type="date" disabled class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-gray-50" />
          </div>
          <div v-if="form.transfer_type === 'Loan'">
            <label class="block text-sm font-medium text-gray-700 mb-1">Ngày trả dự kiến</label>
            <input v-model="form.expected_return_date" type="date" :disabled="!isEditable" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm disabled:bg-gray-50" />
          </div>
        </div>
        ...
        <div v-if="isEditable" class="flex justify-end gap-2 pt-2 border-t border-gray-100">
          <button @click="router.push('/asset-transfers')" class="px-4 py-2 text-sm border border-gray-300 rounded-lg hover:bg-gray-50">Quay lại</button>
          <button @click="save" :disabled="saving" class="px-4 py-2 text-sm bg-blue-600 text-white rounded-lg disabled:opacity-50 hover:bg-blue-700">
      ...
      <div v-if="showRejectModal" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
        <div class="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md mx-4">
          ...
          <div class="flex justify-end gap-2">
            <button @click="showRejectModal = false; rejectionReason = ''" class="px-4 py-2 text-sm border border-gray-300 rounded-lg hover:bg-gray-50">Hủy</button>
            <button @click="reject" class="px-4 py-2 text-sm bg-red-600 text-white rounded-lg hover:bg-red-700">Xác nhận từ chối</button>
          </div>
        </div>
      </div>
    </Teleport>
