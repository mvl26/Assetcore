        ...
        <tbody class="divide-y divide-gray-100">
          <tr v-for="d in items" :key="d.name" class="hover:bg-gray-50">
            ...
            <td class="px-4 py-3 text-right space-x-2 whitespace-nowrap">
              <button @click="openEdit(d.name)" class="text-blue-600 text-xs font-medium">Sửa</button>
              <button @click="remove(d.name)" class="text-red-600 text-xs font-medium">Xóa</button>
            </td>
          </tr>
        </tbody>
      </table>
