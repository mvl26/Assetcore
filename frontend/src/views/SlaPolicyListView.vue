        ...
        <tbody class="divide-y divide-gray-100">
          <tr v-for="p in policies" :key="p.name" class="hover:bg-gray-50">
            ...
            <td class="px-4 py-3 text-right space-x-2">
              <button @click="openEdit(p.name)" class="text-blue-600 hover:text-blue-800 text-xs font-medium">Sửa</button>
              <button @click="remove(p.name)" class="text-red-600 hover:text-red-800 text-xs font-medium">Xóa</button>
            </td>
          </tr>
        </tbody>
      </table>
      ...
      <div class="bg-white rounded-xl p-6 w-[560px] max-w-full space-y-4">
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <input type="number" v-model.number="form.response_time_minutes" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" />
          </div>
          <div>
