...
v-if="doc.docstatus === 0" class="btn-ghost" :disabled="acting"
                  @click="router.push(`/purchases/${doc.name}/edit`)">
Sửa
</button>
          <button
...
v-if="doc.docstatus === 1 && doc.status === 'Submitted'"
          ...
          <button
v-if="doc.docstatus === 1 && doc.status !== 'Received' && doc.status !== 'Cancelled'"
      ...
      <!-- Toast -->
      <div
v-if="toast" class="mb-4 px-4 py-3 rounded-lg text-sm transition-all"
...
class="font-medium text-blue-600 hover:underline text-left"
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
        <div
v-if="doc.devices && doc.devices.length"
...
v-if="doc.items && doc.items.length"
      ...
      <div v-if="doc && doc.devices && doc.devices.length" class="card p-5 mb-4">
        ...
        <div class="space-y-3">
          <div
v-for="(d, idx) in doc.devices" :key="d.name || idx"
      ...
      <div v-if="doc.devices && doc.devices.length" class="card p-5 mb-4">
        ...
        <div v-else class="overflow-x-auto">
          <table class="w-full text-sm">
            ...
            <tbody>
              <tr
v-for="c in commissionings" :key="c.name"
      ...
      <div v-if="doc.items && doc.items.length" class="card p-5">
        ...
        <div v-else class="overflow-x-auto">
          <table class="w-full text-sm">
            ...
            <tbody>
              <tr
v-for="m in movements" :key="m.name"
                  class="border-b border-slate-50 hover:bg-slate-50 cursor-pointer transition-colors"
...
v-if="showReceiptModal"
        ...
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 animate-fade-in">
          ...
          <div class="mb-4">
            ...
            <SmartSelect
id="receipt-wh" v-model="receiptWarehouse"
                         doctype="AC Warehouse" placeholder="Chọn kho nhập..." />
          </div>

          <div class="flex items-center gap-2 mb-6">
            <input
id="receipt-auto-submit" v-model="receiptAutoSubmit" type="checkbox"
          ...
          <div class="flex gap-3 justify-end">
            <button
class="btn-ghost" :disabled="creatingReceipt"
                    @click="showReceiptModal = false; receiptWarehouse = ''">
Huỷ
</button>
            <button
class="btn-primary" :disabled="creatingReceipt || !receiptWarehouse"
