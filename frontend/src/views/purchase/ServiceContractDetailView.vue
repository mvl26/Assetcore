  ...
  <div class="page-container animate-fade-in space-y-5">
    ...
    <div v-else class="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
      ...
      <div class="flex gap-3 pt-4 border-t border-gray-100">
        <button v-if="contract.docstatus !== 1" :disabled="saving" class="bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white px-4 py-2 rounded-lg text-sm font-medium" @click="save">Lưu</button>
        <button v-if="contract.docstatus !== 1" :disabled="saving" class="bg-green-600 hover:bg-green-700 disabled:opacity-50 text-white px-4 py-2 rounded-lg text-sm font-medium" @click="submitContract">Gửi duyệt</button>
        <button class="ml-auto text-sm text-red-600 hover:text-red-800" @click="remove">Xóa</button>
      </div>
    </div>
  </div>
</template>
