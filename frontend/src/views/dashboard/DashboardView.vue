...
<template>
  <div class="page-container animate-fade-in">
    <PageHeader
      title="Tổng quan quản trị thiết bị"
      :subtitle="data ? `Cập nhật lần cuối: ${data.generated_at.slice(0, 19)}` : 'AssetCore · HTM'"
    >
      <template #actions>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      <button
        v-for="(c, i) in kpiCards" :key="i"
        class="kpi-card p-5 flex items-start gap-4 text-left"
      ...
      >
        ...
        <div class="flex-1 min-w-0">
          ...
          <p class="text-3xl font-bold font-display tabular-nums mt-0.5" :class="c.accent">{{ fmt(c.value) }}</p>
        </div>
      </button>
      <!-- Skeleton -->
      <template v-if="!kpi && loading">
        <div v-for="i in 4" :key="i" class="kpi-card p-5" style="--kpi-color: #94a3b8">
