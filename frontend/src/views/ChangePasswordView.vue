    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
      ...
      <div>
        ...
        <div class="relative">
          ...
          />
          <button type="button" class="absolute inset-y-0 right-2 text-xs text-gray-500"
                  @click="showOld = !showOld">{{ showOld ? 'Ẩn' : 'Hiện' }}</button>
        </div>
      </div>
      ...
      <div>
        ...
        <div class="relative">
          ...
          />
          <button type="button" class="absolute inset-y-0 right-2 text-xs text-gray-500"
                  @click="showNew = !showNew">{{ showNew ? 'Ẩn' : 'Hiện' }}</button>
        </div>
      </div>
      ...
      <div>
        ...
        <p v-if="confirmPassword && newPassword !== confirmPassword"
           class="text-xs text-red-600 mt-1">Mật khẩu xác nhận không khớp.</p>
      </div>
