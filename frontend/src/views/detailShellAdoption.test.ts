...
const SPEC_PATH = resolve(SRC, '../../docs/ui-ux/03_DETAIL_PAGE_SHELL.md')
...
describe('AC-UX-071 phần C — adoption phải kèm test trạng thái', () => {
  it('mỗi view đã áp khuôn có <tên>DetailStates.test.ts đặt cạnh nó', () => {
    ...
    for (const p of adopters) {
      const dir = dirname(resolve(SRC, p))
      // `AssetDetailView.vue` → `assetDetailStates.test.ts` · `RCADetailView.vue` → `rcaDetailStates.test.ts`.
      // So khớp KHÔNG phân biệt hoa/thường: quy ước camelCase của repo không đều ở tiền tố
      // viết tắt (RCA, PM, CM, CAPA) — ép chữ hoa chính xác sẽ báo nhầm file đang có thật.
      const want = `${basename(p, '.vue').replace(/DetailView$/, '')}DetailStates.test.ts`.toLowerCase()
      const found = readdirSync(dir).some((e) => e.toLowerCase() === want)
  ...
  it('21 file test của lô 2 được khai đủ ở §13.6', () => {
    ...
    const missing = lot2
      ...
      .map(([dir, f]) => {
        const base = basename(f, '.vue').replace(/DetailView$/, '')
        return `${dir}/${base.charAt(0).toLowerCase()}${base.slice(1)}DetailStates.test.ts`
      })
