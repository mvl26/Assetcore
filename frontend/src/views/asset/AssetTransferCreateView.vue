...
const { clear: clearDraft } = useFormDraft('asset-transfer-create', form)
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <DateInput id="at-date" v-model="form.transfer_date" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" />
        </div>
        <div v-if="isLoan">
          ...
          <DateInput id="at-return-date" v-model="form.expected_return_date" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" />
        </div>
      </div>
      ...
      <button
        ...
        @click="submit"
      >
{{ saving ? 'Đang tạo...' : 'Tạo phiếu điều chuyển thiết bị' }}
</button>
    </div>
  </div>
</template>
