...
function makeDevice(model = '', cost = 0): CreatePurchaseDevicePayload {
  return {
    ...
    vendor_serial_no: '', warranty_months: 12, clinical_dept: '', notes: '',
  }
}
  ...
  <div class="page-container animate-fade-in">
    ...
    <div class="mb-6">
      ...
      <p class="text-xs text-slate-500 mt-1">
        PO có thể chứa <b>thiết bị y tế</b> (→ phiếu tiếp nhận) và/hoặc <b>phụ tùng</b> (→ phiếu nhập kho).
      </p>
    </div>
    ...
    <div class="card p-5 mb-4">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input id="pur-date" v-model="purchaseDate" type="datetime-local" class="form-input w-full" />
        </div>
        <div>
          <label for="pur-invoice" class="form-label">Số hóa đơn / Mã PO</label>
          <input id="pur-invoice" v-model="invoiceNo" type="text" class="form-input w-full font-mono"
                 placeholder="VD: INV-2026-0001, PO-00123" />
        </div>
        <div>
          <label for="pur-delivery" class="form-label">Ngày giao hàng dự kiến</label>
          <input id="pur-delivery" v-model="expectedDelivery" type="date" class="form-input w-full" />
        </div>
      </div>
    </div>
    ...
    <div class="card p-5 mb-4">
      ...
      <div class="mb-4 p-3 rounded-lg border border-dashed border-indigo-200 bg-indigo-50/40">
        ...
        <div class="grid grid-cols-12 gap-2 items-end">
          ...
          <div class="col-span-4 md:col-span-2">
            ...
            <input id="qa-count" v-model.number="quickCount" type="number" min="1" max="50" step="1"
                   class="form-input w-full text-sm" />
          </div>
          <div class="col-span-4 md:col-span-3">
            <label for="qa-cost" class="text-[10px] text-slate-500 mb-1 block">Đơn giá / máy</label>
            <input id="qa-cost" v-model.number="quickCost" type="number" min="0" step="1000"
                   class="form-input w-full text-sm" />
          </div>
          <div class="col-span-4 md:col-span-2">
            <button
      ...
      <div v-else class="space-y-3">
        <div v-for="(d, idx) in devices" :key="idx"
          ...
          <div class="col-span-6 md:col-span-2">
            ...
            <input :id="`dev-cost-${idx}`" v-model.number="d.unit_cost" type="number" min="0" step="1000"
                   class="form-input w-full text-sm" />
          </div>
          <div class="col-span-5 md:col-span-3">
            <label :for="`dev-wm-${idx}`" class="text-[10px] text-slate-500 mb-1 block">BH (tháng)</label>
            <input :id="`dev-wm-${idx}`" v-model.number="d.warranty_months" type="number" min="0" step="1"
                   class="form-input w-full text-sm" />
          </div>
          <div class="col-span-1 flex items-end pb-0.5 justify-end">
            <button class="p-2 text-red-500 hover:text-red-700 hover:bg-red-50 rounded transition-colors"
                    title="Xóa máy này"
                    @click="removeDevice(idx)">✕</button>
          </div>
          <div class="col-span-12 md:col-span-6">
            <label :for="`dev-sn-${idx}`" class="text-[10px] text-slate-500 mb-1 block">Serial Number Hãng (máy #{{ idx + 1 }})</label>
            <input :id="`dev-sn-${idx}`" v-model="d.vendor_serial_no" type="text"
                   class="form-input w-full text-sm font-mono" placeholder="Có thể bỏ trống, điền khi tiếp nhận" />
          </div>
          <div class="col-span-12 md:col-span-6">
            <p class="text-[10px] text-slate-500 mb-1">Khoa sử dụng</p>
            <SmartSelect v-model="d.clinical_dept" doctype="AC Department" placeholder="Chọn khoa..." />
          </div>
        </div>
    ...
    <div class="card p-5 mb-4">
      ...
      <div v-else class="space-y-3">
        <div v-for="(row, idx) in items" :key="idx"
          ...
          <div class="col-span-12 md:col-span-5 relative">
            ...
            <div v-else class="relative">
              <input :id="`pur-part-${idx}`" type="text"
            ...
            <!-- No results -->
            <div v-else-if="activeRowIdx === idx && !searchLoading && searchQuery.length >= 2 && !row.spare_part"
          ...
          <div class="col-span-4 md:col-span-2">
            ...
            <input :id="`pur-qty-${idx}`" v-model.number="row.qty" type="number" min="0.01"
          ...
          <div class="col-span-4 md:col-span-2">
            ...
            <input :id="`pur-cost-${idx}`" v-model.number="row.unit_cost" type="number" min="0" step="1000"
                   class="form-input w-full text-sm" />
          </div>
    ...
    <div class="card p-5 mb-4">
      ...
      <textarea id="pur-notes" v-model="notes" rows="3" class="form-input w-full"
