...
describe('CI grep-gate: depreciation_method i18n leak (anti pattern A)', () => {
  it('quét được ≥1 SFC (glob hoạt động — chống false-green do glob rỗng)', () => {
    // Nếu glob trả 0 file → gate vô dụng (luôn pass). Pin số file > 0.
    expect(Object.keys(vueFiles).length).toBeGreaterThan(0)
  })
