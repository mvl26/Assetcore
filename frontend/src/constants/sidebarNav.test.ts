...
import { getPersona, derivePersonas, PERSONAS } from './personas'
...
describe('AppSidebar header — không leak nhãn persona', () => {
  // process.cwd() = frontend/ khi chạy vitest → resolve tới src/components.
  const SRC = readFileSync(
    resolve(process.cwd(), 'src/components/common/AppSidebar.vue'),
    'utf-8',
  )
  ...
  it('UI-SIDEBAR-1a: header dùng brand tĩnh, không bind personaTitle', () => {
    ...
    expect(SRC).toMatch(/BRAND_TITLE\s*=\s*['"]AssetCore['"]/)
    expect(SRC).not.toMatch(/\{\{\s*personaTitle\s*\}\}/)
    expect(SRC).not.toContain('const personaTitle')
  })
  ...
  it('UI-SIDEBAR-1b: không hard-code nhãn persona nào trong template sidebar', () => {
    for (const p of PERSONAS) {
      ...
      expect(SRC).not.toContain(p.label)
    }
  })
